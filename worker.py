...
class CrexMatchWorker:
    ...
    def __init__(self, match_data, ai_api_key):
        ...
        self.keywords = ["yorker", "bouncer", "drive", "pull", "sweep", "cutter", "slow-ball", "edge", "straight", "length"]
    ...
    async def listen(self):
        ...
        if not token:
            print(f"[Error] Could not find token for {self.crex_id}. Retrying in 60s...")
            return

        final_ws_url = f"{self.ws_url}&token={token}"
        
        async with websockets.connect(final_ws_url, extra_headers=stealth_session.get_headers()) as ws:
            ...
            while self.is_running:
                ...
                if message.startswith("2"):
                    await ws.send("3") # Keep connection alive (Mirroring human behavior)
    ...
    async def generate_ai_commentary(self, ball, flavor):
        """
        Step 5: Calls Hugging Face Inference API to generate 100% original content.
        """
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(url, headers=headers, json={"inputs": prompt, "parameters": {"max_new_tokens": 50, "temperature": 0.7}}, timeout=10)
