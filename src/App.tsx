...
type AppTab = 'home' | 'verdict' | 'momentum' | 'debate' | 'career' | 'smartxi' | 'wallet' | 'live' | 'raffle' | 'blog' | 'prediction' | 'admin';
...
export default function App() {
  ...
  const [walletBalance, setWalletBalance] = useState(500); // Mock balance in INR
  ...
  const [raffleHistory] = useState<RaffleHistory[]>([
    ...
    { drawId: "RD-881", winner: "cricket_fan_1", prize: "₹500 Wallet", date: "Mar 20" },
  ]);
    ...
    try {
      ...
      if (error && error.code === 'PGRST116') {
        ...
        const { data: newProfile, error: createError } = await supabase
          .from('profiles')
          .insert([
            { id: session.user.id, username: session.user.email?.split('@')[0], cricket_iq: 100, wallet_balance: 500 }
          ])
          .select()
          .single();
  ...
  }, [cricketIQ, walletBalance, session, profile]);

  const updateProfileStats = async (newIQ?: number, newWallet?: number) => {
    // Local state updates first
    if (newIQ !== undefined) setCricketIQ(newIQ);
    if (newWallet !== undefined) setWalletBalance(newWallet);
    
    if (!session?.user) return;
    
    const updates: any = {};
    if (newIQ !== undefined) updates.cricket_iq = newIQ;
    if (newWallet !== undefined) updates.wallet_balance = newWallet;
  ...
  const handleBuyTicket = (qty: number = 1) => {
    ...
    */
    ...
    updateProfileStats(undefined, walletBalance - totalCost);
    ...
    <div className="min-h-screen bg-surface text-white font-body selection:bg-primary selection:text-black overflow-x-hidden">
        ...
        <div className="flex justify-between items-center px-4 md:px-8 h-16 w-full max-w-[1920px] mx-auto">
          ...
          <div className="flex items-center space-x-4 md:space-x-6">
            ...
            <button className="p-2 text-gray-400 hover:text-white transition-colors relative">
              <Bell size={20} />
              <span className="absolute top-2 right-2 w-2 h-2 bg-aurora-teal rounded-full border border-black" />
            </button>
      ...
      <main className="pt-24 px-4 pb-28 max-w-[1200px] mx-auto flex flex-col items-center">
        <AnimatePresence mode="wait">
              ...
              <div className="w-full max-w-7xl py-12 space-y-24">
                ...
                <section className="space-y-12">
                  <div className="flex flex-col md:flex-row justify-between items-end gap-6">
                    <div className="space-y-2">
                      <h3 className="text-4xl font-black text-white uppercase italic tracking-tighter">Core Pillars</h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {[
                      ...
                      >
                        <div className={`p-4 bg-white/5 rounded-2xl w-fit group-hover:scale-110 transition-transform ${feature.color}`}>{feature.icon}</div>
                        <div className="space-y-2">
                ...
                <section className="bg-white/[0.02] border border-white/5 rounded-[60px] p-12 md:p-20">
                  <div className="max-w-3xl mx-auto text-center space-y-12">
                    <div className="space-y-4">
                      <h3 className="text-4xl md:text-6xl font-black text-white uppercase italic tracking-tighter">The Ecosystem</h3>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                      {[
                        ...
                        >
                          ...
                          <span className="block text-xs font-black text-white uppercase tracking-widest">{extra.title}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="p-8 rounded-2xl bg-gradient-to-br from-aurora-teal/20 to-transparent border border-aurora-teal/20 space-y-6 relative overflow-hidden group">
                    <div className="absolute -right-4 -top-4 opacity-5 group-hover:opacity-10 transition-opacity">
                      <Zap size={120} className="text-aurora-teal" />
                    </div>
                    ...
                    {true ? (
                      ...
                      <button 
                        onClick={async () => { 
                          ...
                          */
                          if (walletBalance >= 199) {
                            await updateProfileStats(undefined, walletBalance - 199);
                ...
                <div className="bg-[#111111] border border-aurora-teal/30 rounded-3xl p-12 flex flex-col items-center space-y-8">
                  <div className="relative w-32 h-32">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <Zap size={32} className="text-aurora-teal animate-pulse" />
                    </div>
                  </div>
                  <div className="text-center space-y-2">
          ...
          {activeTab === 'blog' && (
            ...
            >
              <div className="flex justify-between items-center">
                <h2 className="text-3xl font-black text-white tracking-tighter uppercase italic">Crinava Notes</h2>
                <button 
          ...
          {activeTab === 'wallet' && (
            ...
            >
              <div className="text-center space-y-2">
                <h2 className="text-3xl font-black text-white uppercase tracking-tighter italic">Crinava Wallet</h2>
                <p className="text-gray-500 text-[10px] font-black uppercase tracking-widest">Secure Digital Assets</p>
              </div>

              <div className="p-10 rounded-3xl bg-gradient-to-br from-aurora-teal/20 to-transparent border border-aurora-teal/20 text-center space-y-6">
                <div className="space-y-1">
                  <div className="text-[10px] text-gray-400 font-black uppercase tracking-widest">Available Balance</div>
                  <div className="text-5xl font-black text-white tracking-tighter">₹{walletBalance}</div>
                </div>
                <div className="flex justify-center gap-2">
                  <div className="w-2 h-2 bg-aurora-teal rounded-full animate-pulse"></div>
                  <span className="text-[8px] text-aurora-teal font-black uppercase tracking-widest">Encrypted Sync</span>
                </div>
              </div>
                ...
                {[100, 500, 1000, 2000].map((amount) => (
                  <button 
                    ...
                    onClick={() => {
                      ...
                      */
                      updateProfileStats(undefined, walletBalance + amount);
                    }}
                    className="p-6 bg-[#111111] border border-white/5 rounded-2xl hover:border-aurora-teal/50 transition-all group"
                  >
                    <div className="text-[10px] text-gray-500 font-black uppercase tracking-widest group-hover:text-aurora-teal">Recharge</div>
                    <div className="text-2xl font-black text-white">₹{amount}</div>
                  </button>
                ))}
              </div>

              <p className="text-[9px] text-gray-600 text-center font-medium italic">
                Transactions are processed via secure gateway. No card data stored.
              </p>
            </motion.div>
          )}
          {activeTab === 'verdict' && (
                  ...
                  >
                    <div className="flex justify-center">
                      <div className="relative w-16 h-16">
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                          <Zap size={24} className="text-metallic-gold animate-pulse" />
                        </div>
                      </div>
                    </div>
                    <div className="space-y-2">
      ...
      <nav className="fixed bottom-0 w-full bg-[#050505] border-t border-white/5 flex justify-around items-end px-2 py-3 z-50 shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
        ...
        <button 
          ...
          className={`flex flex-col items-center gap-1 min-w-[50px] transition-all hover:scale-110 ${activeTab === 'wallet' ? 'text-metallic-gold' : 'text-gray-500'}`}
        >
          <Wallet size={20} />
