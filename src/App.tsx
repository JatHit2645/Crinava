...
const CoinIcon = ({ size = 24, className = "", noShadow = false }: { size?: number, className?: string, noShadow?: boolean }) => (
  ...
  >
    <div className="absolute inset-0 bg-gradient-to-r from-metallic-gold via-metallic-gold to-metallic-gold animate-shimmer" />
    ...
    <div className="absolute inset-0 border-[0.5px] border-metallic-gold/30 rounded-full" />
  </div>
);
...
async function getCricketVerdict(claim: string): Promise<VerdictData> {
  let contextData = {};
  try {
    const summaryDoc = await getDoc(doc(db, 'verdict_engine', 'summary'));
    const topPlayersDoc = await getDoc(doc(db, 'verdict_engine', 'top_players'));
    if (summaryDoc.exists() && topPlayersDoc.exists()) {
      contextData = {
        summary: summaryDoc.data(),
        topPlayers: topPlayersDoc.data()
      };
    }
  } catch (e) {
    console.warn("Could not fetch context data from Firestore", e);
  }

  const response = await fetch('/api/verdict', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ claim, contextData })
  });

  if (!response.ok) {
    throw new Error('Failed to get verdict from engine');
  }
...
const VerdictCard = ({ data, onBack }: { data: VerdictData, onBack: () => void }) => {
  ...
  return (
    ...
    >
      <button 
        onClick={onBack}
        className="flex items-center gap-2 text-gray-500 hover:text-metallic-gold transition-colors mb-4"
      >
        <ArrowLeft size={18} />
        <span className="text-[10px] font-black uppercase tracking-widest">New Analysis</span>
      </button>

      <div className="bg-[#111111] border border-metallic-gold/10 rounded-2xl overflow-hidden shadow-2xl">
        {/* Header */}
        <div className="p-8 border-b border-metallic-gold/5 bg-gradient-to-br from-metallic-gold/[0.02] to-transparent">
          <div className="flex justify-between items-start mb-6">
            ...
            <button className="p-2 bg-metallic-gold/5 rounded-lg text-aurora-teal hover:bg-metallic-gold/10 transition-colors">
              <Share2 size={20} />
            </button>
          </div>
          <h2 className="text-2xl md:text-3xl font-black italic text-metallic-gold leading-tight">
            "{data.claim}"
          </h2>
        </div>
        ...
        <div className="p-8 space-y-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {data.rawStats.map((stat, i) => (
              <div key={i} className="space-y-1">
                <div className="text-xl font-black text-metallic-gold">{stat.value}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {data.contextStats.map((stat, i) => (
              <div key={i} className="p-4 rounded-xl bg-metallic-gold/[0.02] border border-metallic-gold/5 space-y-2">
          ...
          <div className="p-6 rounded-xl bg-metallic-gold/5 border border-metallic-gold/20 relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-2">
              ...
              <div className="text-[11px] font-bold text-metallic-gold/80">{data.surpriseStat.label}</div>
              <p className="text-[10px] text-gray-500 italic font-medium">{data.surpriseStat.context}</p>
            </div>
          </div>

          {/* Nuance */}
          <div className="pt-6 border-t border-metallic-gold/5">
        ...
        {/* Footer */}
        <div className="p-4 bg-metallic-gold/[0.01] border-t border-metallic-gold/5 flex justify-center">
...
export default function App() {
  ...
  const userEmail = session?.user?.email || "hemnanijatin9@gmail.com";
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => {
    const saved = localStorage.getItem('crinava-theme');
    return saved ? saved === 'dark' : true;
  });
  const [notifications, setNotifications] = useState<Notification[]>([
    { id: '1', title: 'Welcome to Crinava!', message: 'Start exploring the new era of cricket analytics.', time: 'Just now', read: false, type: 'success' },
    { id: '2', title: 'Daily Bonus', message: 'You received 50 Crinava Coins as a daily login bonus!', time: '2h ago', read: false, type: 'info' }
  ]);
  ...
  useEffect(() => {
    if (isDarkMode) {
      ...
      localStorage.setItem('crinava-theme', 'light');
    }
  }, [isDarkMode]);
  ...
  const buyCoins = async (amount: number) => {
    try {
      ...
      sendNotification('Purchase Successful', `You've added ${amount} Crinava Coins to your balance.`, 'success');
    } catch (err) {
      console.error(err);
      sendNotification('Purchase Failed', 'There was an error processing your coin purchase.', 'alert');
    }
  };
  ...
  const handleBuyTicket = (qty: number = 1) => {
    ...
    */
    ...
    if (coinBalance < totalCost) {
      ...
      sendNotification('Insufficient Balance', 'You need more coins to buy raffle tickets.', 'warning');
      return;
    }
    ...
    setRaffleQuantity(1);
    sendNotification('Tickets Purchased', `Successfully bought ${qty} raffle ticket(s). Good luck!`, 'success');
  };
  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface font-body selection:bg-primary selection:text-black overflow-x-hidden transition-colors duration-300">
      {/* Header */}
      <header className="fixed top-0 w-full z-50 bg-surface/80 backdrop-blur-xl border-b border-metallic-gold/5 transition-colors duration-300">
        <div className="flex justify-between items-center px-4 md:px-8 h-16 w-full max-w-[1920px] mx-auto">
          ...
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => { setActiveTab('home'); setVerdict(null); setQuery(''); }}>
            ...
            <div className="flex flex-col items-start">
              ...
              <span className="text-[7px] text-gray-400 tracking-[0.3em] font-black uppercase mt-1">The New Era of Cricket</span>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2 md:space-x-4">
            ...
            <div className="hidden sm:flex items-center gap-2">
              <button 
                ...
                className="flex items-center gap-2 px-3 py-1.5 bg-metallic-gold/5 border border-metallic-gold/10 rounded-xl hover:border-metallic-gold/30 transition-all group"
              >
                <CoinIcon size={18} />
                <span className="text-xs font-black text-on-surface tracking-tighter">{coinBalance}</span>
              </button>
              ...
              <AnimatePresence>
                {showIQ && (
                  ...
                  >
                    <span className="text-xs font-black text-on-surface italic tracking-tighter">{cricketIQ} IQ</span>
                  </motion.div>
                )}
              </AnimatePresence>
              <button 
                onClick={() => setShowIQ(!showIQ)}
                className={`p-2 rounded-full border transition-all group ${showIQ ? 'bg-aurora-teal/20 border-aurora-teal' : 'bg-metallic-gold/5 border-metallic-gold/10 hover:border-aurora-teal/30'}`}
              >
            ...
            <button 
              ...
              className="p-2 text-gray-400 hover:text-on-surface transition-colors bg-metallic-gold/5 rounded-xl border border-metallic-gold/10"
            ...
            <div className="relative">
              <button 
                ...
                className={`p-2 transition-colors relative bg-metallic-gold/5 rounded-xl border border-metallic-gold/10 ${showNotifications ? 'text-aurora-teal border-aurora-teal/50' : 'text-gray-400 hover:text-on-surface'}`}
              >
                <Bell size={20} />
                {unreadCount > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-[10px] font-black flex items-center justify-center rounded-full border-2 border-surface text-metallic-gold">
                    {unreadCount}
                  </span>
                )}
              </button>
              ...
              <AnimatePresence>
                {showNotifications && (
                  <motion.div 
                    ...
                    className="absolute right-0 mt-2 w-80 bg-surface border border-metallic-gold/10 rounded-2xl shadow-2xl overflow-hidden z-[60]"
                  >
                    <div className="p-4 border-b border-metallic-gold/5 flex justify-between items-center bg-metallic-gold/[0.02]">
                    ...
                    <div className="max-h-[400px] overflow-y-auto">
                      {notifications.length === 0 ? (
                        ...
                        notifications.map((n) => (
                          <div 
                            key={n.id} 
                            className={`p-4 border-b border-metallic-gold/5 hover:bg-metallic-gold/[0.02] transition-colors flex gap-3 ${!n.read ? 'bg-aurora-teal/[0.02]' : ''}`}
                          >
                            <div className={`mt-1 w-2 h-2 rounded-full shrink-0 ${
                              n.type === 'success' ? 'bg-emerald-500' : 
                              n.type === 'warning' ? 'bg-metallic-gold' : 
                              n.type === 'alert' ? 'bg-red-500' : 'bg-aurora-teal'
                            }`} />
                            <div className="space-y-1">
                              <h4 className="text-[11px] font-black text-on-surface leading-none">{n.title}</h4>
                              <p className="text-[10px] text-gray-400 leading-relaxed">{n.message}</p>
                              <span className="text-[8px] text-gray-600 font-bold uppercase">{n.time}</span>
                            </div>
                          </div>
                        ))
                      )}
            ...
            <button 
              ...
              className="p-2 text-gray-400 hover:text-on-surface transition-colors bg-metallic-gold/5 rounded-xl border border-metallic-gold/10"
            >
              <UserCircle size={22} className={session ? "text-aurora-teal" : ""} />
            </button>
          </div>
        </div>
      </header>
      ...
      <main className="pt-24 px-4 pb-28 max-w-[1200px] mx-auto flex flex-col items-center">
        <AnimatePresence mode="wait">
          {activeTab === 'home' && (
            ...
            >
              <div className="w-full max-w-7xl py-12 space-y-24">
                ...
                <section className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                  <div className="space-y-8 order-2 lg:order-1">
                    <div className="space-y-4">
                      ...
                      <h2 className="text-7xl md:text-9xl font-black text-metallic-gold tracking-tighter uppercase leading-[0.8] italic">
                        CRICKET <br/>
                    ...
                    <div className="flex gap-6 pt-4">
                      <button 
                        ...
                        className="px-12 py-5 bg-metallic-gold text-black font-black text-sm uppercase tracking-widest rounded-2xl hover:bg-aurora-teal transition-all shadow-[0_20px_40px_rgba(0,229,255,0.2)]"
                      >
                        Launch Oracle
                      </button>
                      <button 
                        onClick={() => setActiveTab('blog')}
                        className="px-12 py-5 bg-metallic-gold/5 border border-metallic-gold/10 text-metallic-gold font-black text-sm uppercase tracking-widest rounded-2xl hover:bg-metallic-gold/10 transition-all"
                      >
                        Read Notes
                      </button>
                    </div>
                  ...
                  <div className="relative order-1 lg:order-2">
                    <div className="aspect-[4/5] rounded-[60px] overflow-hidden border border-metallic-gold/10 shadow-2xl group">
                      <img 
                      ...
                      <div className="absolute bottom-8 left-8 right-8 p-6 bg-black/40 backdrop-blur-md border border-metallic-gold/10 rounded-3xl">
                        <div className="flex justify-between items-center">
                          <div>
                            <div className="text-[10px] text-aurora-teal font-black uppercase tracking-widest mb-1">Live Telemetry</div>
                            <div className="text-xl font-black text-metallic-gold uppercase italic">Lord's Stadium</div>
                          </div>
                ...
                <section className="space-y-12">
                  <div className="flex flex-col md:flex-row justify-between items-end gap-6">
                    <div className="space-y-2">
                      <h3 className="text-4xl font-black text-metallic-gold uppercase italic tracking-tighter">Core Pillars</h3>
                      <p className="text-gray-500 text-sm font-medium">Advanced tools for deep match understanding.</p>
                    </div>
                    <div className="h-px flex-1 bg-metallic-gold/5 mx-8 hidden md:block" />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    {[
                      ...
                      <div 
                        ...
                        className="p-10 rounded-[40px] bg-[#111111] border border-metallic-gold/5 space-y-6 cursor-pointer hover:border-aurora-teal/30 hover:bg-[#151515] transition-all group"
                      >
                        <div className={`p-4 bg-metallic-gold/5 rounded-2xl w-fit group-hover:scale-110 transition-transform ${feature.color}`}>{feature.icon}</div>
                        <div className="space-y-2">
                          <h4 className="text-2xl font-black text-metallic-gold italic uppercase tracking-tighter">{feature.title}</h4>
                ...
                {/* Secondary Features */}
                <section className="bg-metallic-gold/[0.02] border border-metallic-gold/5 rounded-[60px] p-12 md:p-20">
                  <div className="max-w-3xl mx-auto text-center space-y-12">
                    <div className="space-y-4">
                      <h3 className="text-4xl md:text-6xl font-black text-metallic-gold uppercase italic tracking-tighter">The Ecosystem</h3>
                      <p className="text-gray-400 text-lg font-medium">Beyond analysis. A complete cricket experience.</p>
                    </div>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                      {[
                        { icon: <TrendingUp size={20} />, title: 'Live', tab: 'live' },
                        { icon: <Trophy size={20} />, title: 'Raffle', tab: 'raffle' },
                        ...
                        >
                          <div className="w-16 h-16 mx-auto bg-metallic-gold/5 rounded-2xl flex items-center justify-center text-aurora-teal group-hover:bg-aurora-teal group-hover:text-black transition-all">
                            {extra.icon}
                          </div>
                          <span className="block text-xs font-black text-metallic-gold uppercase tracking-widest">{extra.title}</span>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          {activeTab === 'momentum' && (
            ...
            >
              <div className="text-center space-y-2">
                <h2 className="text-4xl font-black italic text-metallic-gold uppercase tracking-tighter">Momentum Map</h2>
                <p className="text-aurora-teal text-xs font-black uppercase tracking-widest">Pressure Wave Analysis</p>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="md:col-span-2 p-6 rounded-3xl bg-[#111111] border border-metallic-gold/5 space-y-6">
                  <div className="flex justify-between items-center">
                    <select 
                      onChange={(e) => generateMomentumData(e.target.value)}
                      className="bg-metallic-gold/5 border border-metallic-gold/10 text-metallic-gold text-xs font-bold rounded-lg px-4 py-2 outline-none focus:border-aurora-teal/50"
                    >
                    ...
                    {selectedMatch && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black text-metallic-gold uppercase tracking-widest">Telemetry Active</span>
                      </div>
                    )}
                  </div>
                  ...
                  {momentumData.length > 0 ? (
                    ...
                    <div className="h-[300px] flex flex-col items-center justify-center border border-dashed border-metallic-gold/10 rounded-2xl space-y-4">
                ...
                <div className="space-y-6">
                  <div className="p-6 rounded-3xl bg-[#111111] border border-metallic-gold/5 space-y-4">
                    <h3 className="text-xs font-black text-aurora-teal uppercase tracking-widest">Impact Analysis</h3>
                    {selectedMatch ? (
                      <div className="space-y-4">
                        <div className="p-4 rounded-2xl bg-metallic-gold/[0.02] border border-metallic-gold/5 space-y-2">
                          <div className="flex justify-between items-center">
                          ...
                          <p className="text-xs text-metallic-gold font-bold italic">"The moment it was won: Dhoni's consecutive sixes shifted pressure by 84%."</p>
                        </div>
                        <div className="flex items-center gap-3 p-3 rounded-xl bg-aurora-teal/10 border border-aurora-teal/20">
                          ...
                          <div>
                            ...
                            <div className="text-xs font-black text-metallic-gold">MS Dhoni (+42.5)</div>
                          </div>
                        </div>
                      </div>
                    ) : (
                  ...
                  <button className="w-full py-4 bg-metallic-gold/5 border border-metallic-gold/10 rounded-2xl flex items-center justify-center gap-2 hover:bg-metallic-gold/10 transition-all group">
                    <Share2 size={16} className="text-gray-400 group-hover:text-aurora-teal" />
                    <span className="text-[10px] font-black text-metallic-gold uppercase tracking-widest">Share Story Card</span>
                  </button>
                </div>
              </div>
            </motion.div>
          ...
          {activeTab === 'debate' && (
            ...
            >
              <div className="flex justify-between items-end">
                <div className="space-y-1">
                  <h2 className="text-4xl font-black italic text-metallic-gold uppercase tracking-tighter">Debate Room</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {debates.map((d) => (
                  <div key={d.id} className="p-6 rounded-3xl bg-[#111111] border border-metallic-gold/5 space-y-6 flex flex-col">
                    <div className="flex justify-between items-start">
                      <div className="p-2 bg-metallic-gold/5 rounded-lg">
                    ...
                    <h3 className="text-lg font-black text-metallic-gold italic leading-tight">"{d.claim}"</h3>
                    ...
                    <div className="space-y-4 mt-auto">
                      <div className="flex justify-between text-[10px] font-black text-metallic-gold uppercase italic">
                      ...
                      <div className="h-2 w-full bg-metallic-gold/5 rounded-full overflow-hidden flex">
                        <div 
                      ...
                      {d.userVote ? (
                        <div className="p-3 rounded-xl bg-metallic-gold/5 border border-metallic-gold/10 text-center">
                          <span className="text-[10px] font-black text-aurora-teal uppercase tracking-widest">You voted {d.userVote}</span>
                        </div>
                      ) : (
                        <div className="flex gap-2">
                          ...
                          <button 
                            ...
                            className="flex-1 py-2 bg-red-500/10 border border-red-500/20 text-red-400 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-red-500 hover:text-metallic-gold transition-all"
                          >
                            Vote Against
                          </button>
                        </div>
          ...
          {activeTab === 'career' && (
            ...
            >
              <div className="text-center space-y-2">
                <h2 className="text-4xl font-black italic text-metallic-gold uppercase tracking-tighter">Career Story</h2>
                <p className="text-aurora-teal text-xs font-black uppercase tracking-widest">The Deep Diver Journey</p>
              </div>

              <div className="flex justify-center">
                <div className="flex bg-metallic-gold/5 p-1 rounded-2xl border border-metallic-gold/10">
                  {['Virat Kohli', 'Sachin Tendulkar', 'MS Dhoni'].map((p) => (
                    <button 
                      ...
                      className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${careerPlayer === p ? 'bg-aurora-teal text-black' : 'text-gray-400 hover:text-metallic-gold'}`}
                    >
                      {p}
                    </button>
                  ))}
              ...
              {careerPlayer ? (
                <div className="space-y-12">
                  {/* Timeline */}
                  <div className="p-8 rounded-3xl bg-[#111111] border border-metallic-gold/5 space-y-8">
                    <div className="flex justify-between items-center">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {(careerData?.chapters || []).map((chapter, i) => (
                      <div key={i} className="p-6 rounded-2xl bg-metallic-gold/[0.02] border border-metallic-gold/5 space-y-3 hover:border-aurora-teal/30 transition-all">
                        <div className="text-[8px] font-black text-aurora-teal uppercase tracking-[0.2em]">{chapter.year}</div>
                        <h4 className="text-sm font-black text-metallic-gold italic">{chapter.title}</h4>
                  ...
                  <div className="flex justify-center gap-4">
                    <button className="px-8 py-3 bg-metallic-gold/5 border border-metallic-gold/10 text-metallic-gold font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-metallic-gold/10 transition-all flex items-center gap-2">
                ...
                <div className="py-20 flex flex-col items-center justify-center space-y-4">
                  <div className="w-16 h-16 rounded-full bg-metallic-gold/5 flex items-center justify-center border border-dashed border-metallic-gold/10">
          ...
          {activeTab === 'smartxi' && (
            ...
            >
              <div className="text-center space-y-2">
                <h2 className="text-4xl font-black italic text-metallic-gold uppercase tracking-tighter">Smart XI</h2>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 space-y-6">
                  <div className="p-6 rounded-3xl bg-[#111111] border border-metallic-gold/5 space-y-6">
                    <div className="flex justify-between items-center">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                      {Array.from({ length: 11 }).map((_, i) => (
                        <div 
                          key={i} 
                          className={`aspect-[3/4] rounded-xl border flex flex-col items-center justify-center p-2 text-center transition-all ${selectedSmartXI[i] ? 'bg-aurora-teal/10 border-aurora-teal/30' : 'bg-metallic-gold/[0.01] border-dashed border-metallic-gold/10'}`}
                        >
                          {selectedSmartXI[i] ? (
                            <>
                              <div className="text-[8px] font-black text-aurora-teal uppercase mb-1">{selectedSmartXI[i].role}</div>
                              <div className="text-[10px] font-black text-metallic-gold leading-tight">{selectedSmartXI[i].name}</div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                      {mockPlayers.map((player) => (
                        <div 
                          ...
                          className={`p-3 rounded-xl border flex justify-between items-center cursor-pointer transition-all ${selectedSmartXI.find(p => p.id === player.id) ? 'opacity-30 pointer-events-none' : 'bg-metallic-gold/[0.02] border-metallic-gold/5 hover:border-aurora-teal/30'}`}
                        >
                          <div>
                            <div className="text-xs font-black text-metallic-gold">{player.name}</div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        {['Sehwag', 'Gavaskar', 'Rohit'].map((opt) => (
                          <button key={opt} className="w-full p-3 rounded-xl bg-metallic-gold/5 border border-metallic-gold/10 text-left text-[10px] font-black text-metallic-gold hover:bg-metallic-gold/10 transition-all flex justify-between">
                            {opt}
                  ...
                  <button className="w-full py-4 bg-metallic-gold/5 border border-metallic-gold/10 rounded-2xl flex items-center justify-center gap-2 hover:bg-metallic-gold/10 transition-all group">
                    ...
                    <span className="text-[10px] font-black text-metallic-gold uppercase tracking-widest">Compare with Friends</span>
                  </button>
                </div>
              </div>
            </motion.div>
          ...
          {activeTab === 'live' && (
            ...
            >
              <div className="flex justify-between items-center mb-8">
                <div className="space-y-1">
                  <h2 className="text-3xl font-black italic text-metallic-gold uppercase tracking-tighter">Live Telemetry</h2>
                  <p className="text-aurora-teal text-[10px] font-black uppercase tracking-widest">Global Match Sync Active</p>
                </div>
                <button 
                  onClick={fetchLiveScores}
                  className="p-2 bg-metallic-gold/5 rounded-lg hover:bg-metallic-gold/10 transition-all"
                >
              ...
              {loading ? (
                ...
                <div className="grid grid-cols-1 gap-4">
                  {liveMatches.map((match, i) => (
                    <div key={i} className="p-6 bg-[#111111] border border-metallic-gold/5 rounded-2xl space-y-4 hover:border-aurora-teal/30 transition-all group">
                      <div className="flex justify-between items-start">
                        <div className="space-y-1">
                          ...
                          <h3 className="text-lg font-black text-metallic-gold uppercase italic">{match.teams.join(' vs ')}</h3>
                          <div className="text-[9px] text-gray-500 font-medium">{match.venue} • {match.format}</div>
                        </div>
                        <div className="px-3 py-1 bg-metallic-gold/5 rounded-lg text-[10px] font-black text-metallic-gold uppercase tracking-widest">
                          {match.status}
                        </div>
                      </div>
                      <div className="text-2xl font-black text-metallic-gold tracking-tighter">
                        {match.score}
                      </div>
                    </div>
                  ))}
                ...
                <div className="text-center py-20 bg-[#111111] rounded-3xl border border-metallic-gold/5">
              ...
              {!prediction && !simulating ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="p-8 rounded-2xl bg-gradient-to-br from-aurora-teal/20 to-transparent border border-aurora-teal/20 space-y-6 relative overflow-hidden group">
                    ...
                    <h3 className="text-xl font-black text-metallic-gold italic">Oracle Simulation</h3>
                    ...
                    {true ? (
                      <div className="space-y-4">
                        <input 
                          ...
                          className="w-full bg-black/40 border border-metallic-gold/10 rounded-xl px-4 py-3 text-xs focus:border-aurora-teal outline-none transition-all"
                  ...
                  <div className="p-8 rounded-2xl bg-[#111111] border border-metallic-gold/10 space-y-6 relative overflow-hidden group">
                    <div className="absolute -right-4 -top-4 opacity-5 group-hover:opacity-10 transition-opacity">
                      <Trophy size={120} className="text-metallic-gold" />
                    </div>
                    <h3 className="text-xl font-black text-metallic-gold italic">Prediction Game</h3>
                    <p className="text-[10px] text-gray-500 font-medium leading-relaxed">
                ...
                <div className="bg-[#111111] border border-aurora-teal/30 rounded-3xl p-12 flex flex-col items-center space-y-8">
                  <div className="relative w-32 h-32">
                    <svg className="w-full h-full" viewBox="0 0 100 100">
                      <circle className="text-metallic-gold/5 stroke-current" strokeWidth="4" fill="transparent" r="45" cx="50" cy="50" />
                      <circle 
                  ...
                  <div className="text-center space-y-2">
                    <h3 className="text-xl font-black text-metallic-gold uppercase italic tracking-tighter">Simulating Reality</h3>
                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <h3 className="text-2xl font-black text-metallic-gold uppercase italic">{prediction.match}</h3>
                      <div className="text-[10px] text-aurora-teal font-black uppercase tracking-widest">Simulation Complete • 1M Iterations</div>
                    </div>
                    <button onClick={() => setPrediction(null)} className="text-gray-500 hover:text-metallic-gold">
                      <ArrowLeft size={20} />
                    </button>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="p-4 bg-metallic-gold/5 rounded-2xl border border-metallic-gold/5">
                    ...
                    <div className="p-4 bg-metallic-gold/5 rounded-2xl border border-metallic-gold/5">
                      ...
                      <div className="text-xl font-black text-metallic-gold">{prediction.probability}%</div>
                    </div>
                    <div className="p-4 bg-metallic-gold/5 rounded-2xl border border-metallic-gold/5">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                      <div className="space-y-1">
                        ...
                        <div className="text-[10px] font-bold text-metallic-gold/80">{prediction.factors.toss}</div>
                      </div>
                      <div className="space-y-1">
                        <div className="text-[8px] text-gray-500 font-black uppercase tracking-widest">Weather</div>
                        <div className="text-[10px] font-bold text-metallic-gold/80">{prediction.factors.weather}</div>
                      </div>
                      <div className="space-y-1">
                        <div className="text-[8px] text-gray-500 font-black uppercase tracking-widest">Pitch</div>
                        <div className="text-[10px] font-bold text-metallic-gold/80">{prediction.factors.pitch}</div>
                      </div>
                      <div className="space-y-1">
                        <div className="text-[8px] text-gray-500 font-black uppercase tracking-widest">Wind</div>
                        <div className="text-[10px] font-bold text-metallic-gold/80">{prediction.factors.wind || 'N/A'}</div>
                      </div>
                      <div className="space-y-1">
                        <div className="text-[8px] text-gray-500 font-black uppercase tracking-widest">Humidity</div>
                        <div className="text-[10px] font-bold text-metallic-gold/80">{prediction.factors.humidity || 'N/A'}</div>
                      </div>
                    </div>
                  </div>

                  <div className="p-6 bg-black/40 rounded-2xl border border-metallic-gold/5">
            ...
            >
              <div className="bg-gradient-to-br from-metallic-gold/20 to-transparent p-8 rounded-3xl border border-metallic-gold/20 relative overflow-hidden">
                <div className="relative z-10 space-y-6">
                  <div className="flex justify-between items-start">
                    <div className="space-y-1">
                      <h2 className="text-3xl font-black text-metallic-gold tracking-tighter uppercase italic">IPL Raffle</h2>
                      <p className="text-metallic-gold text-[10px] font-black uppercase tracking-widest">Next Draw: 2h 45m</p>
                    </div>
                    <div className="bg-black/40 px-4 py-2 rounded-xl border border-metallic-gold/10">
                      <span className="text-2xl font-black text-metallic-gold">10 Coins</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <h3 className="text-[10px] font-black text-metallic-gold uppercase tracking-widest">My Tickets ({raffleTickets.length})</h3>
                  ...
                  {raffleTickets.length > 0 ? (
                    <div className="grid grid-cols-2 gap-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                      {raffleTickets.map((ticket, i) => (
                        <div key={i} className="p-2 bg-metallic-gold/5 border border-metallic-gold/10 rounded-lg text-center">
                          <span className="text-[9px] font-black text-metallic-gold font-mono tracking-tighter">{ticket}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="p-8 bg-[#111111] border border-metallic-gold/5 rounded-2xl text-center">
                ...
                <div className="space-y-4">
                  <h3 className="text-[10px] font-black text-metallic-gold uppercase tracking-widest">Recent Winners</h3>
                  <div className="space-y-3">
                    {raffleHistory.map((item, i) => (
                      <div key={i} className="p-4 bg-[#111111] border border-metallic-gold/5 rounded-2xl flex justify-between items-center">
                        <div>
                          <div className="text-[10px] font-black text-metallic-gold uppercase">{item.winner}</div>
                ...
                {isRaffleModalOpen && (
                  <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
                    ...
                    <motion.div 
                      ...
                      className="relative w-full max-w-sm bg-[#0a0a0a] border border-metallic-gold/10 rounded-3xl p-8 space-y-8 shadow-2xl"
                    >
                      <div className="text-center space-y-2">
                        <h3 className="text-2xl font-black text-metallic-gold uppercase italic tracking-tighter">Buy Tickets</h3>
                      ...
                      <div className="flex items-center justify-center gap-8">
                        <button 
                          ...
                          className="w-12 h-12 rounded-full border border-metallic-gold/10 flex items-center justify-center text-metallic-gold hover:bg-metallic-gold/5"
                        >
                          -
                        </button>
                        <span className="text-4xl font-black text-metallic-gold">{raffleQuantity}</span>
                        <button 
                      ...
                      <div className="p-4 bg-metallic-gold/5 rounded-2xl flex justify-between items-center">
                      ...
                      <div className="flex gap-3">
                        <button 
                          onClick={() => setIsRaffleModalOpen(false)}
                          className="flex-1 py-4 border border-metallic-gold/10 text-metallic-gold font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-metallic-gold/5"
                        >
                          Cancel
                        </button>
                        <button 
          ...
          {activeTab === 'blog' && (
            ...
            >
              <div className="flex justify-between items-center">
                <h2 className="text-3xl font-black text-metallic-gold tracking-tighter uppercase italic">Crinava Notes</h2>
                <button 
              ...
              <div className="grid grid-cols-1 gap-6">
                {blogPosts.map((post, i) => (
                  <div key={i} className="group cursor-pointer space-y-4 p-8 rounded-3xl bg-[#111111] hover:bg-metallic-gold/[0.02] transition-all border border-metallic-gold/5 hover:border-aurora-teal/30">
                    <div className="flex justify-between items-center">
                      <div className="flex items-center gap-3">
                        <span className="px-3 py-1 bg-metallic-gold/5 text-gray-400 text-[8px] font-black uppercase tracking-widest rounded-full">{post.category}</span>
                        {post.isAI && (
                    ...
                    <h3 className="text-2xl font-black text-metallic-gold group-hover:text-aurora-teal transition-colors leading-tight italic">
                      {post.title}
                    </h3>
          ...
          {activeTab === 'store' && (
            ...
            >
              <div className="text-center space-y-3">
                <h2 className="text-5xl font-black text-metallic-gold uppercase tracking-tighter italic">Crinava Store</h2>
              ...
              <div className="relative group">
                ...
                <div className="relative p-12 rounded-[40px] bg-[#0A0A0A] border border-metallic-gold/5 text-center space-y-8 overflow-hidden">
                  ...
                  <div className="space-y-2 relative z-10">
                    ...
                    <div className="text-7xl font-black text-metallic-gold tracking-tighter flex items-center justify-center gap-4">
                      <CoinIcon size={64} />
                      {coinBalance}
                    </div>
                  </div>
                ...
                {[
                  ...
                  <button 
                    ...
                    className={`relative p-8 rounded-[32px] border transition-all duration-500 group overflow-hidden ${
                      pkg.highlight 
                        ? 'bg-metallic-gold/5 border-aurora-teal/30 hover:border-aurora-teal' 
                        : 'bg-[#0D0D0D] border-metallic-gold/5 hover:border-metallic-gold/20'
                    }`}
                  >
                    {pkg.highlight && (
                    ...
                    <div className="flex justify-between items-end">
                      <div className="space-y-1">
                        <div className="text-3xl font-black text-metallic-gold flex items-center gap-2">
                          <CoinIcon size={28} />
                          {pkg.amount}
                        </div>
                        <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Crinava Coins</div>
                      </div>
                      <div className="text-right">
                        <div className="text-xl font-black text-metallic-gold">₹{pkg.price}</div>
                        <div className="text-[8px] text-gray-600 font-black uppercase tracking-widest">One-time</div>
                      </div>
                    </div>
                    
                    <div className="mt-6 w-full py-3 bg-metallic-gold/5 border border-metallic-gold/10 rounded-xl text-[10px] font-black text-metallic-gold uppercase tracking-widest group-hover:bg-metallic-gold/10 transition-all">
                      Purchase Now
                    </div>
                  </button>
                ))}
              ...
              <div className="pt-8 text-center space-y-4">
                ...
                <div className="flex justify-center gap-8 opacity-20 grayscale">
                  <div className="text-[10px] font-black uppercase tracking-widest text-metallic-gold">Visa</div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-metallic-gold">Mastercard</div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-metallic-gold">UPI</div>
                </div>
              </div>
            </motion.div>
          )}
          {activeTab === 'verdict' && (
            ...
            >
              ...
              <section className="text-center space-y-4">
                ...
                <h2 className="text-5xl md:text-7xl font-black text-metallic-gold tracking-tighter uppercase italic leading-none">
              ...
              <div className="max-w-md mx-auto w-full space-y-6">
                <div className="relative group">
                  ...
                  <select 
                    ...
                    className="relative w-full bg-[#111111] border border-metallic-gold/10 text-metallic-gold text-sm font-bold rounded-xl px-6 py-4 outline-none focus:border-aurora-teal/50 appearance-none cursor-pointer"
                  >
                ...
                {isSimulating && (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <h3 className="text-sm font-black text-metallic-gold uppercase tracking-[0.2em]">Oracle Engine Active</h3>
                      <p className="text-[10px] text-aurora-teal font-black uppercase tracking-widest animate-pulse">Running 5,000,000 Iterations...</p>
                    </div>
                    <div className="w-full bg-metallic-gold/5 h-1 rounded-full overflow-hidden">
                      <motion.div 
                        initial={{ width: "0%" }}
                        animate={{ width: "100%" }}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {Object.entries(vertexResult.win_probability).map(([team, prob]: any) => (
                        <div key={team} className="p-8 rounded-[40px] bg-[#111111] border border-metallic-gold/5 text-center group hover:border-aurora-teal/30 transition-all">
                          <p className="text-[10px] text-gray-500 font-black uppercase tracking-[0.2em] mb-3">{team}</p>
                          <p className="text-5xl font-black text-metallic-gold italic tracking-tighter">{prob}%</p>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="p-10 rounded-[50px] bg-gradient-to-br from-aurora-teal/10 to-transparent border border-aurora-teal/20 relative overflow-hidden">
                      ...
                      <p className="text-xl font-medium text-metallic-gold/90 leading-relaxed italic mb-8">
                        "{vertexResult.verdict}"
                      </p>
                      <div className="flex justify-between items-center pt-6 border-t border-metallic-gold/5">
                        <div className="space-y-1">
                          <p className="text-[8px] text-gray-500 font-black uppercase tracking-widest">Iterations</p>
                          <p className="text-xs font-black text-metallic-gold font-mono">{vertexResult.iterations.toLocaleString()}</p>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {vertexResult.key_insights.map((insight: any, i: number) => (
                        <div key={i} className="p-6 rounded-3xl bg-[#111111] border border-metallic-gold/5 space-y-3">
                ...
                {!isSimulating && !vertexResult && (
                  <div className="py-20 text-center space-y-4 opacity-20">
                    <div className="flex justify-center">
                      <Target size={48} className="text-metallic-gold" />
                    </div>
                    <p className="text-xs font-black uppercase tracking-[0.3em] text-metallic-gold">Awaiting Match Telemetry</p>
                  </div>
                )}
              </div>
            </motion.div>
          ...
          {activeTab === 'admin' && isAdmin && (
            ...
            >
              <div className="flex justify-between items-end">
                <div className="space-y-2">
                  <h2 className="text-4xl font-black text-metallic-gold tracking-tighter uppercase italic">Admin Console</h2>
                  <p className="text-aurora-teal text-[10px] font-black uppercase tracking-widest">System Overlord Access</p>
                </div>
                <div className="px-4 py-2 bg-metallic-gold/5 border border-metallic-gold/10 rounded-xl">
                  <span className="text-[8px] text-gray-500 font-black uppercase tracking-widest block">Logged in as</span>
                  <span className="text-[10px] text-metallic-gold font-bold">{userEmail}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className="p-8 bg-[#111111] border border-metallic-gold/5 rounded-3xl space-y-6">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-xl font-black text-metallic-gold uppercase italic">Create Blog</h3>
                    <p className="text-xs text-gray-500">Publish new Crinava Notes to the community.</p>
                  </div>
                  <button className="w-full py-3 bg-metallic-gold/5 border border-metallic-gold/10 text-metallic-gold text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-metallic-gold/10 transition-all">
                    Open Editor
                  </button>
                </div>
                ...
                <div className="p-8 bg-[#111111] border border-metallic-gold/5 rounded-3xl space-y-6">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-xl font-black text-metallic-gold uppercase italic">Tournaments</h3>
                ...
                <div className="p-8 bg-[#111111] border border-metallic-gold/5 rounded-3xl space-y-6">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-xl font-black text-metallic-gold uppercase italic">Moderation</h3>
              ...
              <div className="p-8 bg-gradient-to-r from-aurora-teal/10 to-transparent border border-metallic-gold/5 rounded-3xl">
                <div className="flex justify-between items-center">
                  <div className="space-y-1">
                    <h4 className="text-lg font-black text-metallic-gold uppercase italic">System Health</h4>
      ...
      <nav className="fixed bottom-0 w-full bg-[#050505] border-t border-metallic-gold/5 flex justify-around items-end px-2 py-3 z-50 shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
        <button 
