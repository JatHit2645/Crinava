...
export function calculateBallByBallMomentum(
  ...
  let immediate_impact = event.is_wicket
    ? config.wicketImpact
    : event.runs_scored;
  if (event.is_boundary) immediate_impact += config.boundaryBonus;

  const current_es_impact =
    immediate_impact * config.alpha + previous_es_impact * (1.0 - config.alpha);

  // Normalize based on config
  const normalized_score =
    (current_es_impact / (config.baselineCRR / 6.0)) * 100.0;

  return {
    score: Math.max(
      -100,
      Math.min(100, Math.round(normalized_score * 10) / 10),
    ),
