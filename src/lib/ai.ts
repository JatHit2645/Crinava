...
console.log("ai.ts: API Key found:", !!apiKey, apiKey ? `(Prefix: ${apiKey.substring(0, 10)}...)` : "(NONE)");

let baseURL = process.env.NVIDIA_API_URL || "https://integrate.api.nvidia.com/v1";
console.log("ai.ts: Raw NVIDIA_API_URL:", process.env.NVIDIA_API_URL || "(DEFAULT)");
...
export const aiClient = {
  chat: {
    completions: {
      create: () => {
        throw new Error("SDK_USAGE_DETECTED: The OpenAI SDK is being used instead of direct fetch!");
      }
    }
  },
  baseURL: baseURL
