    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        if (playersData && playersData.length > 0) {
          ...
          if (careerStats && (careerStats.total_runs > 0 || careerStats.total_wickets > 0)) {
            setStats({
              ...
              average: careerStats.total_dismissals > 0 
                ? (careerStats.total_runs / careerStats.total_dismissals).toFixed(2) 
                : careerStats.total_runs.toFixed(2),
              strikeRate: careerStats.total_balls_faced > 0 
                ? ((careerStats.total_runs / careerStats.total_balls_faced) * 100).toFixed(2) 
                : '0.00',
              wickets: careerStats.total_wickets,
              bowlingAverage: careerStats.total_wickets > 0 
                ? (careerStats.total_runs_conceded / careerStats.total_wickets).toFixed(2) 
                : 'N/A',
              economy: careerStats.total_balls_bowled > 0 
                ? (careerStats.total_runs_conceded / (careerStats.total_balls_bowled / 6)).toFixed(2) 
                : '0.00',
            ...
            console.log("DEBUG: Career stats empty, aggregating from deliveries for:", dbPlayerId);
            const { data: deliveries, error: delError } = await supabase
              .from('deliveries')
              .select('runs_batter, runs_total, wicket_kind, batter_id, bowler_id')
              .or(`batter_id.eq.${dbPlayerId},bowler_id.eq.${dbPlayerId}`);
            
            if (delError) throw delError;

            let runs = 0, ballsFaced = 0, wickets = 0, ballsBowled = 0, runsConceded = 0;
            ...
            setStats({
              ...
              runs,
              ballsFaced,
              average: ballsFaced > 0 ? (runs / (ballsFaced / 6)).toFixed(2) : '0.00', // Simplified
          ...
          setStats({
            ...
            runs: 0, ballsFaced: 0, average: '0.00', strikeRate: '0.00',
            wickets: 0, bowlingAverage: 'N/A', economy: '0.00', overs: '0.0'
          });
        }
      } catch (error) {
    ...
    >
      ...
      <div className="flex items-center justify-between bg-black/40 p-6 rounded-2xl border border-white/10 backdrop-blur-md">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h1 className="text-3xl font-black text-white uppercase italic">{stats?.name}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-black/40 p-6 rounded-2xl border border-white/10 backdrop-blur-md space-y-6">
          <div className="flex items-center gap-3 text-metallic-gold">
            ...
            <h2 className="text-lg font-black uppercase tracking-widest">Batting Career</h2>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/5 p-4 rounded-xl">
              <div className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Total Runs</div>
              <div className="text-3xl font-black text-white">{stats?.runs}</div>
            </div>
            <div className="bg-white/5 p-4 rounded-xl">
              <div className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Strike Rate</div>
              <div className="text-3xl font-black text-white">{stats?.strikeRate}</div>
            </div>
            <div className="bg-white/5 p-4 rounded-xl">
              <div className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Average</div>
              <div className="text-3xl font-black text-white">{stats?.average}</div>
            </div>
            <div className="bg-white/5 p-4 rounded-xl">
              <div className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Balls Faced</div>
              <div className="text-3xl font-black text-white">{stats?.ballsFaced}</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-black/40 p-6 rounded-2xl border border-white/10 backdrop-blur-md space-y-6">
          <div className="flex items-center gap-3 text-aurora-teal">
            ...
            <h2 className="text-lg font-black uppercase tracking-widest">Bowling Career</h2>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-white/5 p-4 rounded-xl">
              <div className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Wickets</div>
              <div className="text-3xl font-black text-white">{stats?.wickets}</div>
            </div>
            <div className="bg-white/5 p-4 rounded-xl">
              <div className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Economy</div>
              <div className="text-3xl font-black text-white">{stats?.economy}</div>
            </div>
            <div className="bg-white/5 p-4 rounded-xl">
              <div className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Average</div>
              <div className="text-3xl font-black text-white">{stats?.bowlingAverage}</div>
            </div>
            <div className="bg-white/5 p-4 rounded-xl">
              <div className="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Overs</div>
              <div className="text-3xl font-black text-white">{stats?.overs}</div>
            </div>
          </div>
        </div>
      </div>
