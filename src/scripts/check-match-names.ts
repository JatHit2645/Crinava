...
async function checkMatchScorecard() {
  ...
  const rawInfo = typeof match.raw_info === 'string' ? JSON.parse(match.raw_info) : match.raw_info;
  ...
  console.log('Sample delivery batter:', firstOver?.deliveries?.[0]?.batter || firstOver?.batter);
}
