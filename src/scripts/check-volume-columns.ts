...
async function checkVolumeColumns() {
  ...
  const { data, error } = await supabase.from('player_volume_stats').select('*').limit(1);
  if (data && data.length > 0) {
    console.log('Columns:', Object.keys(data[0]));
  } else {
    console.log('Table is empty, trying to find schema info...');
    // Try to query a common column
    const { error: err2 } = await supabase.from('player_volume_stats').select('player_id').limit(1);
