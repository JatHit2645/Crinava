    ...
    <div className="fixed bottom-0 left-0 right-0 bg-[#111111] border-t border-white/10 p-4 flex items-center justify-between z-50">
      <div className="flex gap-2 overflow-x-auto">
        {selectedPlayerIds.map((id) => (
          <div key={id} className="bg-white/10 px-3 py-1 rounded-full flex items-center gap-2 text-sm text-white">
            <span>{id}</span>
            <button onClick={() => removePlayer(id)} className="text-gray-400 hover:text-white">
              <X size={14} />
            </button>
          </div>
        ))}
