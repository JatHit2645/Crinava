...
import { Trophy, MapPin, Calendar, ArrowLeft, Loader2, ChevronRight, Star, Info, Activity, Coins, Search, Filter, SlidersHorizontal, ChevronDown, Check, Zap, Gavel, BookOpen, Users, TrendingUp } from 'lucide-react';
...
const calculateFallbackMvp = (scorecard: any[]) => {
  ...
  scorecard.forEach(inning => {
    inning.batters?.forEach((b: any) => {
      if (!players[b.name]) players[b.name] = { player_name: b.name, total_impact_score: 0 };
      players[b.name].total_impact_score += (b.runs || 0) * 1 + (b.sixes || 0) * 2 - (b.balls || 0) * 0.1;
    });
    inning.bowlers?.forEach((b: any) => {
      if (!players[b.name]) players[b.name] = { player_name: b.name, total_impact_score: 0 };
      players[b.name].total_impact_score += (b.wickets || 0) * 25 + (b.dots || 0) * 1 - (b.runs || 0) * 0.5;
    });
  });
  return Object.values(players).sort((a, b) => b.total_impact_score - a.total_impact_score);
};
...
const TournamentsList: React.FC<{ onSelect: (t: Tournament) => void }> = ({ onSelect }) => {
  ...
  useEffect(() => {
    const fetchAllSeries = async () => {
      ...
      try {
        ...
        if (!response.ok) throw new Error('Failed to fetch series from CockroachDB');
        const allData = await response.json();
        
        console.log(`Successfully loaded ${allData.length} series via CockroachDB.`);
        setAllSeries(allData);
      } catch (err: any) {
        console.error("Fetch error:", err);
        setError(`Failed to load data: ${err.message || 'Unknown error'}. Please check your connection.`);
      }
  ...
  useEffect(() => {
    const filterAndSort = () => {
      const getSortDate = (t: Tournament) => {
        ...
        if (years.length > 0) {
          ...
          if (t.start_date && new Date(t.start_date).getFullYear() === maxYear) {
            return t.start_date;
          }
      ...
      if (genderFilter !== 'all') {
        if (genderFilter === 'female') {
          filtered = filtered.filter(t => t.event_name.toLowerCase().includes('women'));
      ...
      if (formatFilter !== 'all') {
        if (formatFilter === 'IPL') {
          filtered = filtered.filter(t => t.event_name.toLowerCase().includes('ipl') || t.event_name.toLowerCase().includes('indian premier league'));
        } else if (formatFilter === 'BBL') {
          filtered = filtered.filter(t => t.event_name.toLowerCase().includes('bbl') || t.event_name.toLowerCase().includes('big bash'));
        } else {
          filtered = filtered.filter(t => t.event_name.toUpperCase().includes(formatFilter.toUpperCase()) || (t.match_type && t.match_type.toUpperCase() === formatFilter.toUpperCase()));
        }
      }
    ...
    filterAndSort();
  }, [allSeries, searchQuery, sortBy, leaguesOnly, genderFilter, formatFilter, yearRange]);
      ...
      <div className="flex flex-col space-y-4 mb-6">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1 group">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-metallic-gold transition-colors" size={18} />
            <input 
              type="text" 
              ...
              <div className="p-4 bg-[#111111] border border-white/10 rounded-2xl space-y-6">
                ...
                <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5">
                  <div className="space-y-1">
                    <div className="text-xs font-bold text-white">Major Leagues Only</div>
                    <div className="text-[9px] text-gray-500 uppercase tracking-widest">IPL, BBL, Leagues</div>
                  </div>
                  <button 
                  ...
                  >
                    <div className={`absolute top-1 w-3 h-3 rounded-full transition-all ${leaguesOnly ? 'left-6 bg-black' : 'left-1 bg-white'}`} />
                  </button>
                </div>
                ...
                {/* Format Filter */}
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-gray-500">Format / League</label>
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      { id: 'all', label: 'All' },
                      { id: 'T20', label: 'T20I' },
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-black uppercase tracking-widest text-gray-500">Year Range</label>
                    <span className="text-xs font-bold text-metallic-gold">{yearRange[0]} - {yearRange[1]}</span>
                  </div>
                  <div className="px-2">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 space-y-6">
          ...
          <div className="text-center space-y-2">
            <p className="text-white font-black uppercase tracking-[0.2em]">Synchronizing Database</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {tournaments?.map((t, idx) => (
            ...
            >
              <div>
                <h3 className="text-xl font-black text-white group-hover:text-metallic-gold transition-colors">{t.event_name}</h3>
          ...
          {tournaments.length === 0 && (
            <div className="col-span-full py-20 text-center space-y-4">
              <div className="p-4 bg-white/5 rounded-full w-fit mx-auto">
                {error ? <div className="text-red-500 font-black text-2xl">!</div> : <Search size={32} className="text-gray-600" />}
              </div>
              <div className="space-y-1">
                {error ? (
                  <div className="space-y-2">
                    <p className="text-red-500 font-bold">Failed to load series</p>
                    <p className="text-xs text-gray-500 max-w-md mx-auto">{error}</p>
                  </div>
                ) : (
                  <>
                    <p className="text-gray-500 italic">No series found matching your filters.</p>
                    {allSeries.length > 0 && (
                      <p className="text-[10px] text-gray-600 uppercase tracking-widest">
                        Found {allSeries.length} series total, but filters are hiding them.
                      </p>
                    )}
                    {allSeries.length === 0 && (
...
const TournamentMatchesList: React.FC<{ tournament: Tournament; onBack: () => void; onSelectMatch: (id: number) => void }> = ({ tournament, onBack, onSelectMatch }) => {
  ...
  const [activeTab, setActiveTab] = useState<'matches' | 'predict' | 'verdict' | 'analytics' | 'playingXI' | 'stories' | 'stats'>('matches');
  ...
  useEffect(() => {
    const fetchMatches = async () => {
      setLoading(true);
      try {
        console.log(`📡 Fetching matches for: ${tournament.event_name} (${tournament.season})`);
        ...
        if (!response.ok) throw new Error('Failed to fetch matches from CockroachDB');
    ...
    if (activeTab === 'stats' && !stats && matches.length > 0) {
      const fetchStats = async () => {
        ...
        try {
          ...
          console.log("Stats tab requested for historical matches via Crinava Engine.");
          // We set stats to empty for now to avoid the UI hanging
          setStats({
            runs: [], wickets: [], highestScore: [], mostCenturies: [], mostFours: [],
            mostSixes: [], most5WicketHauls: [], bowlingAvg: [], bowlingSR: [],
            highestSR: [], highestAvg: [], bestEcon: [], mvp: []
          });
        } catch (err) {
  ...
  useEffect(() => {
    if (['predict', 'playingXI', 'stories'].includes(activeTab) && !aiContent[activeTab]) {
      const fetchAiContent = async () => {
        ...
        try {
          ...
          setAiContent(prev => ({ ...prev, [activeTab]: response.text || "No content generated." }));
        } catch (err) {
  ...
  if (loading) return <div className="flex justify-center py-20"><Loader2 className="animate-spin text-metallic-gold" size={32} /></div>;
  ...
  const renderStatList = (list: any[], valueKey: string, formatFn?: (p: any) => string | number) => (
    <div className="space-y-2 mt-4">
      {list?.map((p, i) => (
        <div key={i} className="flex justify-between items-center bg-white/5 p-3 rounded-lg border border-white/5">
          <div className="flex items-center gap-3 min-w-0 flex-1">
            <span className="text-gray-500 font-bold w-4 shrink-0">{i + 1}</span>
      ...
      {list?.length === 0 && <div className="text-gray-500 text-sm text-center py-4">Not enough data</div>}
    </div>
  );
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
      ...
      {activeTab === 'stats' && (
        <div className="bg-[#111111] border border-white/10 rounded-2xl p-5 shadow-2xl">
          <h3 className="text-lg font-black italic text-white mb-4 flex items-center gap-2">
            <Trophy size={18} className="text-metallic-gold" /> Tournament Leaders
          </h3>
          
          {statsLoading ? (
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
              <Loader2 className="animate-spin text-metallic-gold" size={32} />
              <p className="text-gray-500 text-xs font-black uppercase tracking-widest animate-pulse">Calculating tournament statistics...</p>
            </div>
          ) : stats ? (
            <>
            ...
            <div className="text-center text-gray-500 py-10 italic">Select the Stats tab to calculate tournament leaders.</div>
          )}
        </div>
      )}
          ...
          {matches?.map((match) => (
            ...
            >
              ...
              <div className="mt-3 text-center text-sm text-metallic-gold font-medium">
                {match.outcome_result || (match.outcome?.by?.runs ? `${match.outcome.winner} won by ${match.outcome.by.runs} runs` : match.outcome?.by?.wickets ? `${match.outcome.winner} won by ${match.outcome.by.wickets} wickets` : match.outcome?.result === 'tie' ? 'Match Tied' : match.outcome?.result === 'no result' ? 'No Result' : 'Match Result Unavailable')}
              </div>
            </div>
          ))}
        </div>
      ...
      {activeTab === 'verdict' && (
        <div className="bg-[#111111] border border-white/10 rounded-2xl p-6 shadow-2xl">
          <VerdictTool 
            scope="series" 
            context={{ eventName: tournament.event_name, season: tournament.season }} 
          />
        </div>
      )}
      ...
      {['predict', 'playingXI', 'stories'].includes(activeTab) && (
        <div className="bg-[#111111] border border-white/10 rounded-2xl p-6 shadow-2xl">
          <h3 className="text-xl font-black italic text-white mb-4 flex items-center gap-2 uppercase tracking-tighter">
            ...
            {activeTab === 'playingXI' && <Users className="text-metallic-gold" />}
            {activeTab === 'stories' && <BookOpen className="text-metallic-gold" />}
          ...
          {aiLoading ? (
            <div className="flex flex-col items-center justify-center py-12 space-y-4">
              <Loader2 className="animate-spin text-metallic-gold" size={32} />
              <p className="text-gray-500 text-xs font-black uppercase tracking-widest animate-pulse">Oracle is analyzing tournament data...</p>
            </div>
          ) : (
...
const MatchDetail: React.FC<{ matchId: number; onBack: () => void }> = ({ matchId, onBack }) => {
  ...
  const [activeTab, setActiveTab] = useState<'scorecard' | 'analytics' | 'verdict'>('analytics');
  const [activeAnalyticsTab, setActiveAnalyticsTab] = useState<'momentum' | 'heatmap' | 'impact' | 'mirror' | 'radar'>('momentum');
  ...
  useEffect(() => {
    const fetchMatchData = async () => {
      setLoading(true);
      try {
        const { getMatchDetails } = await import('../services/cockroachService');
        ...
        if (deliveries && deliveries.length > 0) {
          ...
          deliveries.forEach((d: any) => {
            if (!inningsMap[d.innings]) {
              inningsMap[d.innings] = { team: `Innings ${d.innings}`, batters: {}, bowlers: {}, totalRuns: 0, totalWickets: 0, overs: 0, lastBall: 0 };
            }
            ...
            // Batting
            if (!inn.batters[d.batter]) inn.batters[d.batter] = { name: d.batter, runs: 0, balls: 0, fours: 0, sixes: 0, dismissal: 'not out' };
            ...
            if (d.player_out === d.batter) inn.batters[d.batter].dismissal = d.wicket_kind || 'out';

            // Bowling
            if (!inn.bowlers[d.bowler]) inn.bowlers[d.bowler] = { name: d.bowler, runs: 0, balls: 0, wickets: 0, dots: 0 };
            inn.bowlers[d.bowler].runs += d.runs_total || 0;
            inn.bowlers[d.bowler].balls += 1;
            if (d.runs_total === 0) inn.bowlers[d.bowler].dots += 1;
            if (d.player_out && d.wicket_kind !== 'run out') inn.bowlers[d.bowler].wickets += 1;
          });

          const finalScorecard = Object.values(inningsMap).map((inn: any, idx: number) => ({
            ...inn,
            team: inn.team || `Innings ${idx + 1}`,
            batters: Object.values(inn.batters).map((b: any) => ({ ...b, sr: ((b.runs / (b.balls || 1)) * 100).toFixed(1) })),
            bowlers: Object.values(inn.bowlers).map((b: any) => ({ ...b, econ: ((b.runs / (b.balls || 1)) * 6).toFixed(1) })),
  ...
  if (!match || scorecard.length === 0) return (
    <div className="text-white text-center py-20 space-y-4">
      ...
      <button onClick={onBack} className="text-metallic-gold text-xs font-black uppercase tracking-widest hover:underline">Go Back</button>
    </div>
  );
  ...
  if (match.winner) {
    const outcomeStr = match.outcome_result === 'runs' ? `by ${match.by_runs || ''} runs` : match.outcome_result === 'wickets' ? `by ${match.by_wickets || ''} wickets` : '';
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="p-6 bg-[#111111] border border-white/10 rounded-2xl shadow-2xl space-y-6">
      ...
      <div className="text-center space-y-2">
        <div className="flex justify-center items-center gap-3">
          <div className="text-xs font-black text-aurora-teal uppercase tracking-widest">{match?.match_type || 'T20'}</div>
          <div className="w-1 h-1 rounded-full bg-white/20" />
          <div className="text-xs font-black text-aurora-teal uppercase tracking-widest">
            {match?.match_date ? new Date(match.match_date).getFullYear() : 'Archive'}
          </div>
        </div>
        <h2 className="text-2xl font-black italic text-white">{team1} <span className="text-gray-500 text-lg">vs</span> {team2}</h2>
        <div className="text-sm text-metallic-gold font-bold">{matchResult}</div>
      </div>
          ...
          <div className="space-y-4">
            ...
            <AnimatePresence>
              {showTop5 && (
                ...
                >
                  <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Top 5 Impact Players</div>
                  {mvp?.slice(1)?.map((p: any, i: number) => (
                    <div key={i} className="flex justify-between items-center bg-white/5 p-2 rounded">
                      <span className="text-sm text-white font-medium">{p.player_name}</span>
                      <span className="text-sm font-black text-metallic-gold">{Number(p.total_impact_score).toFixed(1)}</span>
                    </div>
                  ))}
                </motion.div>
              )}
      ...
      <div className="bg-metallic-gold/10 border border-metallic-gold/30 p-4 rounded-xl text-center">
        <div className="text-metallic-gold font-black text-lg">{matchResult}</div>
      </div>
      ...
      {activeTab === 'scorecard' ? (
            ...
            <div className="space-y-4">
              ...
              <div className="bg-[#1a1a1a] border border-white/10 rounded-xl overflow-hidden">
                <div className="bg-white/5 px-4 py-3 border-b border-white/10 flex justify-between items-center">
                  <h4 className="font-bold text-white">{scorecard[activeInning]?.team || 'Unknown Team'} Innings</h4>
                  <span className="font-black text-metallic-gold">{scorecard[activeInning]?.totalRuns || 0}/{scorecard[activeInning]?.totalWickets || 0} <span className="text-xs text-gray-400 font-normal">({scorecard[activeInning]?.overs || '0.0'} ov)</span></span>
                </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {scorecard[activeInning]?.batters?.map((b: any, idx: number) => (
                        <tr key={`${b.name}-${idx}`} className="hover:bg-white/[0.02]">
                          ...
                          <td className="px-4 py-2 text-right text-gray-400">{b.sr}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="overflow-x-auto border-t border-white/10">
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {scorecard[activeInning]?.bowlers?.map((b: any, idx: number) => {
                        ...
                        return (
                          <tr key={`${b.name}-${idx}`} className="hover:bg-white/[0.02]">
                            ...
                            <td className="px-4 py-2 text-right text-gray-400">{b.econ}</td>
                          </tr>
                        );
                      })}
                    </tbody>
        ...
        <div className="space-y-6">
          ...
          <div className="min-h-[500px]">
            {activeAnalyticsTab === 'momentum' && <MomentumMap rawInfo={match.raw_info} />}
            {activeAnalyticsTab === 'heatmap' && <TurningPointHeatmap rawInfo={match.raw_info} />}
            {activeAnalyticsTab === 'impact' && <BallByBallImpact rawInfo={match.raw_info} />}
            {activeAnalyticsTab === 'mirror' && <MirrorMatch rawInfo={match.raw_info} venue={venue || ''} matchType={match.match_type} />}
            {activeAnalyticsTab === 'radar' && (
              <PlayerImpactRadar 
                rawInfo={match.raw_info}
                playerId={mvp && mvp.length > 0 ? mvp[0].player_name : ''} 
                allPlayers={Array.from(new Set(scorecard.flatMap(inning => [
      ...
      <div className="space-y-4">
        ...
        <div className="bg-[#1a1a1a] p-6 rounded-xl border border-white/10 space-y-3">
          <div className="flex justify-between border-b border-white/5 pb-2">
            ...
            <span className="text-white font-medium text-right">{match.toss_winner ? `${match.toss_winner} won the toss and elected to ${match.toss_decision}` : 'N/A'}</span>
          </div>
          <div className="flex justify-between border-b border-white/5 pb-2">
          ...
          <div className="flex justify-between border-b border-white/5 pb-2">
            ...
            <span className="text-white font-medium text-right">{match.match_date || 'N/A'}</span>
          </div>
          <div className="flex justify-between border-b border-white/5 pb-2">
            <span className="text-gray-500">Player of the Match</span>
            <span className="text-metallic-gold font-bold text-right">{match.player_of_match || 'N/A'}</span>
          </div>
          <div className="flex justify-between border-b border-white/5 pb-2">
            <span className="text-gray-500">Umpires</span>
            <span className="text-white font-medium text-right">{match.umpires || 'N/A'}</span>
          </div>
        </div>
      </div>
    </motion.div>
...
const LiveMatchesList: React.FC<{ onSelect: (m: LiveMatch) => void }> = ({ onSelect }) => {
  ...
  useEffect(() => {
    const fetchLive = async () => {
      try {
        ...
        console.error(e);
        setError("Unable to connect to live score engine. Make sure FastAPI server is running on port 7860.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredMatches = liveMatches.filter(m => {
    ...
    const matchesState = stateFilter === 'all' || (m.state || 'Live').toLowerCase() === stateFilter.toLowerCase();
  ...
  if (loading && liveMatches.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        <Loader2 className="animate-spin text-metallic-gold" size={32} />
        <p className="text-gray-500 text-xs font-black uppercase tracking-widest animate-pulse">Scanning live feeds...</p>
      </div>
    );
  }
  ...
  if (liveMatches.length === 0) {
    return (
      <div className="p-12 bg-[#111111] border border-white/5 rounded-3xl text-center space-y-4">
        ...
        <p className="text-xs text-gray-500 font-black uppercase tracking-widest">No active matches found in the discovery engine</p>
        <p className="text-[10px] text-gray-600">Active matches will automatically appear here once detected on Crex/NDTV.</p>
      </div>
    );
  }
        ...
        {(['all', 'live', 'upcoming', 'completed'] as const).map((s) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${
              stateFilter === s
                ? s === 'live' ? 'bg-aurora-teal/20 border-aurora-teal/40 text-aurora-teal'
                : s === 'upcoming' ? 'bg-metallic-gold/20 border-metallic-gold/40 text-metallic-gold'
                : s === 'completed' ? 'bg-blue-500/20 border-blue-500/40 text-blue-400'
            ...
            >
              <div className="absolute top-4 right-4 flex items-center gap-2">
                <span className="flex h-2 w-2 relative">
                  {matchState === 'Live' && <span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${dotColor.split(' ')[0]} opacity-75`}></span>}
                  <span className={`relative inline-flex rounded-full h-2 w-2 ${dotColor.split(' ')[0]}`}></span>
                </span>
                <span className={`text-[8px] font-black uppercase tracking-widest ${dotColor.split(' ')[1]}`}>{matchState}</span>
              </div>

              <div className="space-y-1">
                <div className="text-[8px] font-black text-metallic-gold uppercase tracking-[0.2em]">{match.source} Source</div>
              ...
              <div className="flex justify-between items-center pt-2 border-t border-white/5">
                ...
                <ChevronRight className="text-gray-500 group-hover:text-aurora-teal group-hover:translate-x-1 transition-all" size={16} />
              </div>
            </div>
          );
        }) : (
...
const LiveMatchDetail: React.FC<{ match: LiveMatch; onBack: () => void }> = ({ match, onBack }) => {
  ...
  const [liveTab, setLiveTab] = useState<'commentary' | 'scorecard' | 'graphs'>('commentary');
  const [scorecardData, setScorecardData] = useState<any>(null);
  const [latestTelemetry, setLatestTelemetry] = useState<any>(null);
  const [winHistory, setWinHistory] = useState<{over: string; winA: number; winB: number; teamA: string; teamB: string}[]>([]);
  ...
  useEffect(() => {
    ...
    const addPacket = (packet: LivePacket) => {
      ...
      if (packet.win_predictor) {
        ...
        let winA = 50, winB = 50;
        let teamA = wp.batting_team || wp.teamA || match.title.split(/ vs /i)[0]?.trim() || 'Team 1';
        let teamB = wp.bowling_team || wp.teamB || match.title.split(/ vs /i)[1]?.split(' ')[0]?.trim() || 'Team 2';
        ...
        setWinHistory(prev => {
          const entry = { over: packet.event_key || packet.over_ball, winA, winB, teamA, teamB };
      ...
      setHistory(prev => {
        const packetKey = packet.event_key || `${packet.over_ball}:${packet.runs_scored}`;
        const index = prev.findIndex(p => (p.event_key || `${p.over_ball}:${p.runs_scored}`) === packetKey);
        if (index !== -1) {
  ...
  const latestWin = winHistory.length > 0 ? winHistory[winHistory.length - 1] : null;

  // Helper: render scorecard from backend data
  const renderScorecard = () => {
    if (!scorecardData) return <div className="py-12 text-center text-gray-500 italic bg-white/[0.01] border border-white/5 rounded-3xl">Scorecard loading... Will auto-refresh every 10s.</div>;
    const sc = scorecardData.data || scorecardData;
    const playerMap = scorecardData.player_map || {};
    const resolveName = (id: string) => playerMap[id?.toString()?.toUpperCase()] || playerMap[id] || id || 'Unknown';
    const innings = Array.isArray(sc) ? sc : (sc?.i || sc?.innings || []);
    if (!Array.isArray(innings) || innings.length === 0) return <div className="py-12 text-center text-gray-500 italic bg-white/[0.01] border border-white/5 rounded-3xl">Scorecard data is being fetched by the engine...</div>;
    ...
    const playerLine = (player: any) => {
      ...
      if (player.r !== undefined && player.b !== undefined) return `${name} ${player.r} (${player.b})`;
      if (player.o !== undefined) return `${name} ${player.o}-${player.m ?? 0}-${player.r ?? 0}-${player.w ?? 0}`;
      return name;
    };
    return (
      <div className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
          {[
            ...
            <div key={label} className="bg-white/[0.02] border border-white/5 rounded-2xl p-4">
              <div className="text-[8px] text-gray-500 font-black uppercase tracking-widest">{label}</div>
        ...
        {innings.length > 0 && (
          <div className="flex bg-[#111111] border border-white/10 p-1 rounded-2xl">
            {innings.map((inn: any, idx: number) => {
              const fallbackName = idx === 0 ? (match.team1 || 'Team A') : (match.team2 || 'Team B');
              const tName = resolveName(inn.tn) !== inn.tn ? resolveName(inn.tn) : fallbackName;
              return (
                <button 
                  key={idx}
        ...
        {[innings[activeScorecardInning]].filter(Boolean).map((inn: any) => {
          ...
          const batters = rawBatters.map((bat: any) => {
            if (typeof bat === 'string') {
              ...
              return {
                ...
                f: parts[3], // 4s
                s: parts[4], // 6s
                sr: parts[5] || (parts[2] && Number(parts[2]) > 0 ? ((Number(parts[1]) / Number(parts[2])) * 100).toFixed(1) : '-'),
          ...
          const bowlers = rawBowlers.map((bwl: any) => {
            if (typeof bwl === 'string') {
              ...
              return {
                ...
                r: parts[3],
                w: parts[4],
                e: parts[5] || (parts[1] && Number(parts[1]) > 0 ? (Number(parts[3]) / Number(parts[1])).toFixed(1) : '-')
              };
            }
            return bwl;
          });

          const fallbackTeamName = iIdx === 0 ? (match.team1 || 'Team A') : (match.team2 || 'Team B');
          const teamName = resolveName(inn.tn) !== inn.tn ? resolveName(inn.tn) : fallbackTeamName;
          ...
          return (
            <div key={iIdx} className="bg-white/[0.02] border border-white/5 rounded-2xl overflow-hidden">
              <div className="px-5 py-3 bg-white/[0.03] border-b border-white/5 flex justify-between items-center">
                <span className="font-black text-white text-sm uppercase tracking-wider">{teamName}</span>
                <span className="font-black text-metallic-gold text-sm">{totalScore}/{totalWkts} <span className="text-xs text-gray-400 font-normal">({totalOvers} ov)</span></span>
              </div>
              {extras?.total !== undefined && (
                <div className="px-5 py-2 text-[10px] text-gray-400 border-b border-white/5">
                  Extras: <span className="text-white font-bold">{extras.total}</span>
                  <span className="ml-2">b {extras.byes || 0}, lb {extras.legByes || 0}, w {extras.wides || 0}, nb {extras.noBalls || 0}</span>
                </div>
              )}
              {batters.length > 0 && (
                <div className="overflow-x-auto">
                  <table className="w-full text-xs">
                    <thead><tr className="text-gray-500 border-b border-white/5">
                      <th className="text-left px-4 py-2 font-bold">BATTER</th><th className="px-2 py-2 font-bold">R</th><th className="px-2 py-2 font-bold">B</th><th className="px-2 py-2 font-bold">4s</th><th className="px-2 py-2 font-bold">6s</th><th className="px-2 py-2 font-bold">SR</th>
                    </tr></thead>
                    <tbody>{batters.map((bat: any, bIdx: number) => (
                      <tr key={bIdx} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                        <td className="px-4 py-2">
                          <div className="text-white font-semibold flex items-center gap-2">
                            {resolveName(bat.id || bat.n)}
                            {(!bat.wicket_bowler && bat.r !== undefined) && <span className="text-[9px] text-aurora-teal px-1.5 py-0.5 rounded bg-aurora-teal/10">*</span>}
                          </div>
                          {(bat.wicket_bowler || bat.wicket_fielder) && (
                            <div className="text-[10px] text-gray-500 mt-0.5">
                              {(() => {
                                const bowler = resolveName(bat.wicket_bowler);
                                const fielder = resolveName(bat.wicket_fielder);
                                const type = String(bat.dismissal_type || '');
                                if (type === '1' || type === 'b') return `b ${bowler}`;
                                if (type === '2' || type === 'c') return `c ${fielder} b ${bowler}`;
                                if (type === '3' || type === 'lbw') return `lbw b ${bowler}`;
                                if (type === '4' || type === 'run out') return `run out (${fielder})`;
                                if (type === '5' || type === 'st') return `st ${fielder} b ${bowler}`;
                        ...
                        <td className="px-2 py-2 text-center text-white font-bold">{bat.r ?? '-'}</td>
                        <td className="px-2 py-2 text-center text-gray-400">{bat.b ?? '-'}</td>
                        <td className="px-2 py-2 text-center text-metallic-gold">{bat.f ?? '-'}</td>
                        <td className="px-2 py-2 text-center text-aurora-teal">{bat.s ?? '-'}</td>
                        <td className="px-2 py-2 text-center text-gray-300">{bat.sr ?? '-'}</td>
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
              ...
              {bowlers.length > 0 && (
                <div className="overflow-x-auto border-t border-white/5">
                  <table className="w-full text-xs">
                    <thead><tr className="text-gray-500 border-b border-white/5">
                      <th className="text-left px-4 py-2 font-bold">BOWLER</th><th className="px-2 py-2 font-bold">O</th><th className="px-2 py-2 font-bold">M</th><th className="px-2 py-2 font-bold">R</th><th className="px-2 py-2 font-bold">W</th><th className="px-2 py-2 font-bold">ECON</th>
                    </tr></thead>
                    <tbody>{bowlers.map((bwl: any, bIdx: number) => (
                      <tr key={bIdx} className="border-b border-white/[0.03] hover:bg-white/[0.02]">
                        <td className="px-4 py-2 text-white font-semibold">{resolveName(bwl.id || bwl.n)}</td>
                        <td className="px-2 py-2 text-center text-gray-400">{bwl.o ?? '-'}</td>
                        <td className="px-2 py-2 text-center text-gray-400">{bwl.m ?? '-'}</td>
                        <td className="px-2 py-2 text-center text-white font-bold">{bwl.r ?? '-'}</td>
                        <td className="px-2 py-2 text-center text-red-400 font-bold">{bwl.w ?? '-'}</td>
                        <td className="px-2 py-2 text-center text-gray-300">{bwl.e ?? '-'}</td>
                      </tr>
                    ))}</tbody>
                  </table>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border-t border-white/5 text-xs">
                  {partnerships.length > 0 && (
                    <div className="p-4 border-b md:border-b-0 md:border-r border-white/5">
                      <div className="text-[9px] font-black text-gray-500 uppercase tracking-widest mb-2">Latest Partnerships</div>
                      {partnerships.slice(-3).map((p: any, idx: number) => (
                        <div key={idx} className="flex justify-between py-1 text-gray-300">
                          <span>{p.p1} / {p.p2}</span>
                          <span className="font-black text-white">{p.runs} ({p.balls})</span>
                        </div>
                      ))}
                    </div>
                  )}
                  {dismissals.length > 0 && (
                    <div className="p-4">
                      <div className="text-[9px] font-black text-gray-500 uppercase tracking-widest mb-2">Dismissals</div>
                      {dismissals.slice(-4).map((d: any, idx: number) => (
                        <div key={idx} className="py-1 text-gray-300">
                          <span className="text-white font-semibold">{d.player}</span>
                          {d.bowler ? <span className="text-gray-500"> by {d.bowler}</span> : null}
                        </div>
                      ))}
                    </div>
                  )}
  ...
  const renderGraph = () => {
    if (winHistory.length < 2) return <div className="py-12 text-center text-gray-500 italic bg-white/[0.01] border border-white/5 rounded-3xl">Win prediction graph will appear after 2+ data points are received...</div>;
    const w = 700, h = 250, pad = 40;
    const pw = (w - pad * 2) / (winHistory.length - 1);
    const scaleY = (v: number) => pad + ((100 - v) / 100) * (h - pad * 2);
    const pathA = winHistory.map((p, i) => `${i === 0 ? 'M' : 'L'}${pad + i * pw},${scaleY(p.winA)}`).join(' ');
    const pathB = winHistory.map((p, i) => `${i === 0 ? 'M' : 'L'}${pad + i * pw},${scaleY(p.winB)}`).join(' ');
      ...
      <div className="space-y-4">
        <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4 overflow-x-auto">
          <svg viewBox={`0 0 ${w} ${h}`} className="w-full" style={{ minWidth: 400 }}>
            <line x1={pad} y1={scaleY(50)} x2={w - pad} y2={scaleY(50)} stroke="rgba(255,255,255,0.1)" strokeDasharray="4"/>
            <text x={pad - 5} y={scaleY(100) + 4} fill="#666" fontSize="9" textAnchor="end">100%</text>
            <text x={pad - 5} y={scaleY(50) + 4} fill="#666" fontSize="9" textAnchor="end">50%</text>
            <text x={pad - 5} y={scaleY(0) + 4} fill="#666" fontSize="9" textAnchor="end">0%</text>
            <path d={pathA} fill="none" stroke="#11EBCF" strokeWidth="2.5" strokeLinejoin="round"/>
            <path d={pathB} fill="none" stroke="#FFD700" strokeWidth="2.5" strokeLinejoin="round"/>
            {winHistory.map((p, i) => (
              <g key={i}>
                <circle cx={pad + i * pw} cy={scaleY(p.winA)} r="3" fill="#11EBCF"/>
                <circle cx={pad + i * pw} cy={scaleY(p.winB)} r="3" fill="#FFD700"/>
                {i % Math.max(1, Math.floor(winHistory.length / 8)) === 0 && (
                  <text x={pad + i * pw} y={h - 5} fill="#555" fontSize="8" textAnchor="middle">{p.over}</text>
                )}
              </g>
            ))}
          </svg>
          <div className="flex justify-center gap-6 mt-2 text-xs">
            <span className="flex items-center gap-1.5"><span className="w-3 h-1 rounded bg-aurora-teal inline-block"/><span className="text-gray-400 font-bold">{teamA}</span></span>
            <span className="flex items-center gap-1.5"><span className="w-3 h-1 rounded bg-metallic-gold inline-block"/><span className="text-gray-400 font-bold">{teamB}</span></span>
          </div>
        </div>
        {/* Momentum: runs scored per over as bar chart */}
        <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-4">
          <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest mb-3">Run Flow Per Ball</h4>
          <div className="flex items-end gap-1 h-24 overflow-x-auto custom-scrollbar pb-2">
            {history.filter(p => p.over_ball && !['STAT','UPDATE','Live','Finished'].includes(p.over_ball)).slice().reverse().map((p, i) => {
              const runs = parseInt(p.runs_scored) || 0;
              const isW = p.runs_scored?.toLowerCase().includes('w');
              const barH = isW ? 90 : Math.max(8, (runs / 6) * 90);
              const color = isW ? 'bg-red-500' : runs >= 6 ? 'bg-aurora-teal' : runs >= 4 ? 'bg-metallic-gold' : runs > 0 ? 'bg-white/30' : 'bg-white/10';
              return <div key={i} className="flex flex-col items-center gap-1 min-w-[14px]">
                <div className={`w-3 rounded-t ${color}`} style={{ height: barH + '%' }}/>
                <span className="text-[7px] text-gray-600">{p.over_ball}</span>
              </div>;
            })}
          </div>
        </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
      ...
      <div className="bg-[#111111] border border-white/10 rounded-3xl p-8 relative overflow-hidden shadow-2xl">
        <div className="absolute top-0 right-0 p-8 opacity-5"><Activity size={96} className="text-aurora-teal" /></div>
        <div className="flex flex-col space-y-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-black text-metallic-gold uppercase tracking-[0.2em]">{match.source} Live Feed</span>
              <div className="w-1 h-1 rounded-full bg-white/20" />
              <div className="flex items-center gap-2">
                <span className={`flex h-2 w-2 relative ${connected ? "visible" : "hidden"}`}>
                ...
                <span className={`text-[8px] font-black uppercase tracking-widest ${connected ? "text-aurora-teal" : "text-gray-500 animate-pulse"}`}>
                  {connected ? "STREAMING" : "POLLING FALLBACK"}
                </span>
              </div>
            </div>
          </div>
          <h2 className="text-3xl md:text-4xl font-black italic text-white leading-tight uppercase tracking-tighter">{match.title}</h2>
          <div className="pt-4 border-t border-white/5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div className="space-y-1">
              <div className="text-[9px] text-gray-500 font-black uppercase tracking-widest">Match State</div>
              <div className={`text-2xl font-black italic ${statusColor}`}>{currentScore}</div>
            </div>
            <div className="text-left sm:text-right">
              <div className="text-[9px] text-gray-500 font-black uppercase tracking-widest">Universal Match ID</div>
              <div className="text-xs font-mono text-gray-400 font-bold">{match.match_id}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      {latestWin && (
        <div className="bg-[#111111] border border-white/10 rounded-2xl p-5 space-y-3">
          <div className="text-[9px] font-black text-gray-500 uppercase tracking-widest">Win Probability</div>
          <div className="flex justify-between text-sm font-black">
            <span className="text-aurora-teal">{latestWin.teamA}: {latestWin.winA.toFixed(1)}%</span>
            <span className="text-metallic-gold">{latestWin.teamB}: {latestWin.winB.toFixed(1)}%</span>
          </div>
          <div className="w-full h-3 rounded-full bg-white/5 overflow-hidden flex">
            <div className="h-full bg-gradient-to-r from-[#11EBCF] to-[#11EBCF]/60 transition-all duration-500 flex-none" style={{ width: `${latestWin.winA}%` }}/>
            <div className="h-full bg-gradient-to-l from-metallic-gold to-metallic-gold/60 transition-all duration-500 flex-none" style={{ width: `${latestWin.winB}%` }}/>
          </div>
        </div>
      )}
      ...
      <div className="flex bg-[#111111] border border-white/10 p-1 rounded-2xl">
        {(['commentary', 'scorecard', 'graphs'] as const).map(tab => (
          <button key={tab} onClick={() => setLiveTab(tab)} className={`flex-1 py-2.5 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${liveTab === tab ? 'bg-aurora-teal text-black' : 'text-gray-400 hover:text-white'}`}>
            {tab === 'commentary' ? '💬 Commentary' : tab === 'scorecard' ? '📊 Scorecard' : '📈 Graphs'}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-4">
        {liveTab === 'commentary' && (
          <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
            {history.length > 0 ? history.map((packet, idx) => {
              const isWicket = packet.runs_scored?.toLowerCase().includes('w');
              const isInfo = packet.runs_scored === 'INFO' || packet.runs_scored === 'STAT' || packet.flavor?.includes('update') || packet.flavor?.includes('stat');
              const overLabel = packet.over_ball?.startsWith('UPDATE_') ? 'UPDATE' : packet.over_ball === 'STAT' ? 'STAT' : `OV ${packet.over_ball}`;
              const runLabel = isInfo ? packet.runs_scored : isWicket ? "WKT" : packet.runs_scored === '-' ? 'DOT' : `${packet.runs_scored} RUNS`;
              let runStyle = "bg-white/5 border-white/10 text-white";
              if (isWicket) runStyle = "bg-red-500/20 border-red-500/30 text-red-400";
              else if (isInfo) runStyle = "bg-blue-500/20 border-blue-500/30 text-blue-300";
              else if (packet.runs_scored === '4') runStyle = "bg-metallic-gold/20 border-metallic-gold/30 text-metallic-gold";
              else if (packet.runs_scored === '6') runStyle = "bg-aurora-teal/20 border-aurora-teal/30 text-aurora-teal";
              return (
                <div key={packet.event_key || `${packet.over_ball}-${idx}`} className="p-5 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all flex flex-col md:flex-row gap-4 items-start">
                  <div className="flex items-center gap-3 min-w-[120px]">
                    <div className="text-xs font-black text-gray-400 font-mono">{overLabel}</div>
                    <div className={`px-2.5 py-1 rounded-lg border text-xs font-black uppercase tracking-wider ${runStyle}`}>
                      {runLabel}
                    </div>
                  </div>
                  <div className="flex-1 text-sm text-gray-300 font-medium leading-relaxed">{packet.commentary}</div>
                </div>
              );
            }) : <div className="py-20 text-center text-gray-500 italic bg-white/[0.01] border border-white/5 rounded-3xl">Waiting for commentary updates...</div>}
          </div>
        )}
...
export const MatchesSection: React.FC<{ onBackToHome?: () => void }> = ({ onBackToHome }) => {
  const [view, setView] = useState<'tournaments' | 'matches' | 'detail' | 'live_detail'>('tournaments');
          ...
          <div className="flex bg-[#111111] border border-white/10 p-1 rounded-2xl">
            <button
              ...
              className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                mode === 'live' ? 'bg-aurora-teal text-black font-black' : 'text-gray-400 hover:text-white'
              }`}
            >
              Live Matches
            </button>
            <button
              ...
              className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
                mode === 'archive' ? 'bg-aurora-teal text-black font-black' : 'text-gray-400 hover:text-white'
              }`}
            >
              Archive
            </button>
