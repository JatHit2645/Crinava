...
export const MomentumMap: React.FC<{ rawInfo: any }> = ({ rawInfo }) => {
  ...
  if (loading) return <div className="text-white">Analyzing Pressure Waves...</div>;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-1">
            Pressure Wave Analysis: Visualizing the psychological and tactical shifts in the game's intensity.
          </p>
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data}>
              ...
              <Tooltip 
                ...
                cursor={{ stroke: '#11EBCF', strokeWidth: 1, strokeDasharray: '5 5' }}
              />
              <Line 
                type="monotone" 
                dataKey="pressure" 
                ...
                activeDot={{ r: 5, fill: '#fff', stroke: '#11EBCF', strokeWidth: 2 }}
              />
              <ReferenceLine y={0} stroke="#ffffff40" strokeWidth={2} />
              {data.map((p, i) => p.isTurningPoint && (
                <ReferenceLine 
                  key={i} 
                  x={p.over} 
                  stroke="#FFD700" 
                  strokeDasharray="3 3" 
                  label={{ value: 'Turning Point', position: 'top', fill: '#FFD700', fontSize: 10, fontWeight: 'bold' }} 
                />
              ))}
            </LineChart>
          </ResponsiveContainer>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-aurora-teal/5 rounded-xl border border-aurora-teal/20">
          ...
          <p className="text-xs text-gray-300 leading-relaxed">
            The <span className="text-aurora-teal font-bold uppercase">Pressure Wave</span> represents which team is currently dictating the pace of the game. Positive values indicate dominance by the batting side, while negative values show the bowling side applying intense pressure.
          </p>
        </div>
        <div className="p-4 bg-metallic-gold/5 rounded-xl border border-metallic-gold/20">
          ...
          <p className="text-xs text-gray-300 leading-relaxed">
            Identifying these waves helps in understanding game-changing phases. A <span className="text-metallic-gold font-bold uppercase">Turning Point</span> (gold dashed line) marks a delivery or over where the pressure shifted irreversibly, often deciding the match outcome.
          </p>
        </div>
      </div>
    </div>
