...
import { TrendingUp, X, Zap, Info, ChevronRight, ChevronLeft } from "lucide-react";
...
export const TurningPointHeatmap: React.FC<{ rawInfo: any }> = ({ rawInfo }) => {
  ...
  useEffect(() => {
    ...
    const formattedData: HeatmapData[] = deliveries.map((d: any) => {
      ...
      else if (runs === 0) impactScore = 2; // Dot ball pressure
      else impactScore = 1;
  ...
  if (loading) return (
    <div className="h-[400px] flex flex-col items-center justify-center space-y-4">
      ...
      <div className="text-gray-500 text-xs uppercase font-black tracking-widest animate-pulse">Mapping Match DNA...</div>
    </div>
  );
  ...
  const getCellColor = (d?: HeatmapData) => {
    ...
    if (d.wicket_kind) return 'bg-[#FF4D4D] shadow-[0_0_10px_rgba(255,77,77,0.4)]';
    if (d.impactScore > 7) return 'bg-[#FFD700] shadow-[0_0_10px_rgba(255,215,0,0.4)]';
    if (d.impactScore > 4) return 'bg-[#11EBCF] shadow-[0_0_10px_rgba(17,235,207,0.4)]';
    return 'bg-[#11EBCF]/20';
  };
    ...
    <div className="bg-[#1a1a1a] p-6 rounded-xl border border-white/10 relative overflow-hidden group h-full">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h2 className="text-xl font-black text-white uppercase italic flex items-center gap-2">
            <Zap size={20} className="text-aurora-teal" /> Ball-by-Ball Impact Map
          </h2>
          <p className="text-gray-500 text-xs mt-1">
            Match DNA: A high-resolution heatmap of every delivery and its psychological weight.
          </p>
        </div>
      ...
      <div className="overflow-x-auto pb-4 scrollbar-hide">
        <div className="min-w-[800px]">
          {data.length === 0 ? (
            ...
            <div className="grid grid-cols-[40px_repeat(auto-fit,minmax(30px,1fr))] gap-2" style={{ gridTemplateColumns: `40px repeat(${overs.length}, minmax(30px, 1fr))` }}>
              {/* Header row for Overs */}
              <div />
              {overs.map(o => (
                <div key={o} className="text-[10px] font-black text-gray-600 text-center uppercase">Ov {o}</div>
              ))}

              {/* Rows for Balls */}
              {balls.map(b => (
                <React.Fragment key={b}>
                  <div className="text-[10px] font-black text-gray-600 flex items-center justify-center uppercase">Ball {b}</div>
                  {overs.map(o => {
                    const d = grid[o]?.[b];
                    return (
                      <motion.button
      ...
      <div className="mt-8 flex flex-wrap items-center justify-center gap-6 border-t border-white/5 pt-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-gray-400 uppercase font-black tracking-widest">Wicket</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-[#FFD700] rounded-sm" />
          <span className="text-[10px] text-gray-400 uppercase font-black tracking-widest">Boundary/Six</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-[#11EBCF] rounded-sm" />
          <span className="text-[10px] text-gray-400 uppercase font-black tracking-widest">Four</span>
        </div>
        <div className="flex items-center gap-2">
          <div className="w-3 h-3 bg-[#11EBCF]/20 rounded-sm" />
          <span className="text-[10px] text-gray-400 uppercase font-black tracking-widest">Dot/Single</span>
        </div>
      </div>
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-5 bg-aurora-teal/5 rounded-2xl border border-aurora-teal/20 relative overflow-hidden">
          ...
          <h4 className="text-aurora-teal font-black uppercase tracking-widest text-[10px] mb-2">How to read</h4>
          <p className="text-xs text-gray-300 leading-relaxed font-medium">
            Each cell represents a single delivery. The <span className="text-white font-bold">Intensity</span> is calculated based on runs, wickets, and psychological pressure. Darker/Brighter cells indicate game-defining moments.
          </p>
        </div>
        <div className="p-5 bg-metallic-gold/5 rounded-2xl border border-metallic-gold/20 relative overflow-hidden">
          ...
          <h4 className="text-metallic-gold font-black uppercase tracking-widest text-[10px] mb-2">The "Human" Insight</h4>
          <p className="text-xs text-gray-300 leading-relaxed font-medium">
            Look for clusters of <span className="text-metallic-gold font-bold uppercase">Gold</span> and <span className="text-[#FF4D4D] font-bold uppercase">Red</span>. These are the "Turning Point" phases where the match was effectively decided by individual brilliance or tactical failure.
          </p>
        </div>
      </div>
          ...
          >
            <div className="bg-[#111] border border-aurora-teal/30 p-8 rounded-3xl shadow-2xl max-w-md w-full relative">
              ...
              <div className="text-center space-y-6">
                <div>
                  <div className="text-aurora-teal font-black uppercase tracking-[0.2em] text-[10px] mb-2">Delivery Analysis</div>
                  <div className="text-5xl font-black text-white italic tracking-tighter">Over {selectedBall.over_no}.{selectedBall.ball_no}</div>
                </div>

                <div className={`p-6 rounded-2xl ${selectedBall.wicket_kind ? 'bg-red-500/10 border border-red-500/20' : 'bg-aurora-teal/10 border border-aurora-teal/20'}`}>
                  <div className="text-3xl font-black text-white mb-2">
                    {selectedBall.wicket_kind ? 'WICKET!' : `${selectedBall.runs} Runs`}
                  </div>
