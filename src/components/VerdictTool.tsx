...
import { 
  Send, Loader2, AlertCircle, TrendingUp, CheckCircle2, XCircle, 
  BarChart3, Info, User, Search, Settings2, History, ChevronRight,
  Filter, Database, Zap, HelpCircle, Trophy
} from 'lucide-react';
...
export const VerdictTool: React.FC<VerdictToolProps> = ({ scope, context }) => {
  ...
  useEffect(() => {
    const fetchPlayers = async () => {
      ...
      if (data) {
        setAllPlayers(data.map(p => ({ id: p.player_id, name: p.player_name })));
      }
    };
    fetchPlayers();
  }, []);
  ...
  const getVerdictColor = (verdict: string) => {
    ...
    if (v.includes('INCONCLUSIVE') || v.includes('TIE')) return 'text-yellow-500';
    return 'text-white';
  };
      ...
      <div className="flex p-1 bg-black/40 border border-white/10 rounded-xl w-fit mx-auto mb-8">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-black uppercase tracking-wider transition-all ${
            activeSubTab === 'ask' ? 'bg-aurora-teal text-black' : 'text-gray-500 hover:text-white'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-black uppercase tracking-wider transition-all ${
            activeSubTab === 'know' ? 'bg-aurora-teal text-black' : 'text-gray-500 hover:text-white'
          }`}
        >
        ...
        <div className="lg:col-span-9 space-y-6">
          {activeSubTab === 'ask' ? (
            <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-6 shadow-xl">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="text-lg font-black text-white uppercase tracking-wider">AI Verdict Engine</h2>
                  <p className="text-xs text-gray-400">Natural language analysis for complex cricket queries.</p>
                </div>
              </div>
            ...
            <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-6 shadow-xl">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-lg font-black text-white uppercase tracking-wider">Manual Comparison</h2>
                    <p className="text-xs text-gray-400">Direct data lookup from the 21-dimension engine.</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2 bg-black/40 px-3 py-1.5 rounded-lg border border-white/5">
                    ...
                    <span className="text-[10px] font-bold text-gray-500 uppercase">Min Data: {minDataThreshold} balls</span>
                    <input 
                      type="range" 
                      min="0" 
                      max="1000" 
                      ...
                      onChange={(e) => setMinDataThreshold(parseInt(e.target.value))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Player A</label>
                  <select 
                    value={playerA} 
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Player B</label>
                  <select 
                    value={playerB} 
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Metric</label>
                  <select 
                ...
                onClick={async () => {
                  ...
                  setTimeout(() => {
                    setManualResult({
                      ...
                      playerA: { name: playerA, value: Math.floor(Math.random() * 10000), avg: 4500 },
                      playerB: { name: playerB, value: Math.floor(Math.random() * 10000), avg: 4500 },
                      leagueAvg: 4500
                    });
                    setLoading(false);
                  }, 800);
          ...
          <AnimatePresence mode="wait">
            ...
            {activeSubTab === 'ask' && result && (
              ...
              >
                {/* Verdict Header */}
                <div className={`bg-[#1a1a1a] border border-white/10 rounded-2xl p-8 text-center relative overflow-hidden ${result.outOfScope ? 'opacity-70' : ''}`}>
                  ...
                  <div className="flex justify-center mb-4">
                    {result.verdict.toUpperCase().includes('TRUE') || result.verdict.toUpperCase().includes('SUPERIOR') ? (
                      <CheckCircle2 size={48} className="text-aurora-teal" />
                    ) : result.verdict.toUpperCase().includes('FALSE') || result.verdict.toUpperCase().includes('BELOW') ? (
                  ...
                  <h3 className={`text-4xl font-black italic mb-2 ${getVerdictColor(result.verdict)}`}>
                    {result.verdict}
                  </h3>
                  
                  {!result.outOfScope && (
                    <div className="flex items-center justify-center gap-2">
                      <div className="text-xs font-bold text-gray-500 uppercase tracking-widest">Confidence Score</div>
                ...
                {/* Visual Proof */}
                {result.proof && result.proof.data && result.proof.data.length > 0 && (
                  <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-6">
                    <h4 className="text-xs font-black text-gray-500 uppercase tracking-widest mb-6 flex items-center gap-2">
                      <BarChart3 size={14} /> Visual Proof: {result.proof.title}
                    </h4>
                    ...
                    <div className="h-[300px] w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={result.proof.data}>
                          ...
                          <XAxis dataKey={result.proof.xAxis} stroke="#666" fontSize={12} />
                          <YAxis stroke="#666" fontSize={12} />
                          <Tooltip 
                            contentStyle={{ backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px' }}
                            itemStyle={{ color: '#00C9B7' }}
                          />
                          <Bar dataKey={result.proof.yAxis} fill="#00C9B7" radius={[4, 4, 0, 0]}>
                ...
                <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-8">
                  <div className="flex items-center justify-between mb-8">
                    <h4 className="text-xs font-black text-gray-500 uppercase tracking-widest flex items-center gap-2">
                      <BarChart3 size={14} /> Visual Proof: {selectedMetric.replace('_', ' ').toUpperCase()}
                    </h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-bold text-white">{manualResult.playerA.name}</span>
                        <span className="text-2xl font-black text-aurora-teal">{manualResult.playerA.value}</span>
                      </div>
                      <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                        <motion.div 
                          initial={{ width: 0 }}
                          animate={{ width: `${(manualResult.playerA.value / Math.max(manualResult.playerA.value, manualResult.playerB.value)) * 100}%` }}
                          className="h-full bg-aurora-teal"
                        />
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-gray-500 uppercase">Impact vs Avg:</span>
                        <span className={`text-[10px] font-black ${manualResult.playerA.value > manualResult.leagueAvg ? 'text-aurora-teal' : 'text-red-500'}`}>
                          {manualResult.playerA.value > manualResult.leagueAvg ? '+' : ''}{((manualResult.playerA.value - manualResult.leagueAvg) / manualResult.leagueAvg * 100).toFixed(1)}%
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-bold text-white">{manualResult.playerB.name}</span>
                        <span className="text-2xl font-black text-metallic-gold">{manualResult.playerB.value}</span>
                      </div>
                      <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                        <motion.div 
                          initial={{ width: 0 }}
                          animate={{ width: `${(manualResult.playerB.value / Math.max(manualResult.playerA.value, manualResult.playerB.value)) * 100}%` }}
                          className="h-full bg-metallic-gold"
                        />
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-bold text-gray-500 uppercase">Impact vs Avg:</span>
                        <span className={`text-[10px] font-black ${manualResult.playerB.value > manualResult.leagueAvg ? 'text-aurora-teal' : 'text-red-500'}`}>
                          {manualResult.playerB.value > manualResult.leagueAvg ? '+' : ''}{((manualResult.playerB.value - manualResult.leagueAvg) / manualResult.leagueAvg * 100).toFixed(1)}%
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-center">
                    <button
                      onClick={() => {
                        setActiveSubTab('ask');
                        setQuery(`Compare ${playerA} and ${playerB} on ${selectedMetric.replace('_', ' ')} in ${format} format.`);
                      }}
        ...
        <div className="lg:col-span-3 space-y-6">
            ...
            <div className="space-y-3">
              {recentVerdicts.length > 0 ? (
                recentVerdicts.map((v, i) => (
                  ...
                  >
                    <div className="text-xs text-gray-300 line-clamp-2 mb-1 group-hover:text-white transition-colors">{v}</div>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold text-gray-600 uppercase">Analysis</span>
                      <ChevronRight size={12} className="text-gray-600 group-hover:text-aurora-teal transition-colors" />
                    </div>
                  </button>
                ))
              ) : (
                <div className="text-center py-8">
                  <Search size={24} className="mx-auto text-gray-700 mb-2 opacity-20" />
                  <p className="text-[10px] font-bold text-gray-600 uppercase">No history yet</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-aurora-teal/10 to-purple-500/10 border border-white/10 rounded-2xl p-6 shadow-xl">
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              Use the <span className="text-aurora-teal font-bold">Know Anything</span> tab to find raw data trends, then use <span className="text-aurora-teal font-bold">Explain this verdict</span> to get the AI's deep analysis on that specific data.
            </p>
          </div>
        </div>
      </div>
