...
export const MirrorMatch: React.FC<{ rawInfo: any, venue: string, matchType?: string }> = ({ rawInfo, venue, matchType = 'T20' }) => {
  const [data, setData] = useState<any[]>([]);
  const [selectedOver, setSelectedOver] = useState(6);
  const [selectedMetric, setSelectedMetric] = useState<string>('runs');
  const [loading, setLoading] = useState(true);

  const maxOvers = rawInfo?.info?.overs || (matchType.toUpperCase().includes('ODI') ? 50 : 20);
  ...
  useEffect(() => {
    const fetchVenueData = async () => {
      ...
      try {
        ...
        const processedData = matches.map((match: any) => {
          ...
          if (Array.isArray(info.innings)) {
            ...
            inningsList = Object.values(info.innings).map((inn: any) => Object.values(inn)[0]);
          }
          ...
          inningsList.forEach((inningData: any) => {
            ...
            if (inningData.overs) {
              inningData.overs.forEach((over: any) => {
                if (over.deliveries) deliveries.push(...over.deliveries.map((d: any) => ({ ...d, over_no: over.over + 1 })));
              });
            ...
            deliveries.forEach((d: any) => {
              if (d.over_no <= selectedOver) {
                ...
                const batterRuns = d.runs ? d.runs.batter || d.runs.batsman || 0 : 0;
                matchTotalRuns += r;
                
                if (d.wickets) matchTotalWickets += d.wickets.length;
                if (batterRuns === 4) matchTotalFours += 1;
                if (batterRuns === 6) matchTotalSixes += 1;
                if (batterRuns === 4 || batterRuns === 6) matchTotalBoundaries += 1;
                if (r === 0) matchTotalDotBalls += 1;
                if (d.extras) {
                  matchTotalExtras += (d.extras.wides || 0) + (d.extras.noballs || 0) + (d.extras.legbyes || 0) + (d.extras.byes || 0) + (d.extras.penalty || 0);
                }
          ...
          const runRate = matchTotalLegalBalls > 0 ? (matchTotalRuns / matchTotalLegalBalls) * 6 : 0;
          const dotBallPercent = matchTotalBalls > 0 ? (matchTotalDotBalls / matchTotalBalls) * 100 : 0;
          const runsPerWicket = matchTotalWickets > 0 ? matchTotalRuns / matchTotalWickets : matchTotalRuns;
          const ballsPerBoundary = matchTotalBoundaries > 0 ? matchTotalLegalBalls / matchTotalBoundaries : matchTotalLegalBalls;

          return {
            date: new Date(match.match_date).toLocaleDateString(undefined, { month: 'short', year: '2-digit' }),
            ...
            boundaries: parseFloat((matchTotalBoundaries / numInnings).toFixed(2)),
            sixes: parseFloat((matchTotalSixes / numInnings).toFixed(2)),
            fours: parseFloat((matchTotalFours / numInnings).toFixed(2)),
            dotBalls: parseFloat((matchTotalDotBalls / numInnings).toFixed(2)),
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-xs mt-1 flex items-center gap-1">
            <MapPin size={12} /> Venue Analysis: Historical comparison at {venue}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            >
              {overOptions.map(o => (
                <option key={o} value={o}>Up to Over {o}</option>
              ))}
            </select>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 20, right: 30, left: 0, bottom: 5 }}>
              ...
              <Tooltip 
                ...
                itemStyle={{ fontSize: '12px', fontWeight: 'bold', color: activeMetric.color }}
              ...
              <ReferenceLine 
                ...
                stroke="#ffffff40" 
                strokeDasharray="3 3" 
                label={{ position: 'top', value: 'Venue Avg', fill: '#ffffff80', fontSize: 10 }} 
              />
              <Line 
                type="monotone" 
                ...
                strokeWidth={3}
                dot={{ r: 3, fill: '#1a1a1a', stroke: activeMetric.color, strokeWidth: 2 }}
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-aurora-teal/5 rounded-xl border border-aurora-teal/20">
          ...
          <p className="text-xs text-gray-300 leading-relaxed">
            This chart tracks the <span className="text-white font-bold">average {activeMetric.label.toLowerCase()} per inning</span> across historical matches played at <span className="text-white font-bold">{venue}</span> up to <span className="text-white font-bold">Over {selectedOver}</span>.
          </p>
        </div>
        <div className="p-4 bg-metallic-gold/5 rounded-xl border border-metallic-gold/20">
          ...
          <p className="text-xs text-gray-300 leading-relaxed">
            By comparing historical data at the same venue, you can identify <span className="text-metallic-gold font-bold uppercase">Venue Trends</span> and determine if a team's performance in a specific phase was above or below the venue average.
          </p>
        </div>
      </div>
    </div>
