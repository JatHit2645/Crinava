...
export const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, session }) => {
  ...
  const formatError = (err: any) => {
    ...
    if (message.includes('auth/popup-closed-by-user')) return 'Authentication cancelled';
    if (message.includes('auth/cancelled-popup-request')) return 'Authentication cancelled';
        ...
        >
          <div className="p-8 space-y-8">
            ...
            {error && (
              ...
              >
                <p className="text-[10px] text-cmd-crimson font-black uppercase tracking-widest text-center">{error}</p>
              </motion.div>
            )}
            ...
            {sent ? (
              <div className="space-y-6 text-center py-8">
                ...
                <div className="space-y-2">
                  <h3 className="text-xl font-black text-cmd-text-primary uppercase italic">Check your email</h3>
                  <p className="text-xs text-cmd-text-muted">We've sent a magic link to <span className="text-cmd-text-primary font-bold">{email}</span>.</p>
                </div>
                <button 
              ...
              <div className="space-y-6">
                ...
                <form onSubmit={handleMagicLink} className="space-y-4">
                  <div className="space-y-2">
                    <label className="text-[10px] text-cmd-text-muted font-black uppercase tracking-widest ml-1">Email Address</label>
                    <div className="relative">
                      <Mail className="absolute left-4 top-1/2 -translate-y-1/2 text-cmd-text-muted" size={16} />
                      <input
                        type="email"
                        value={email}
                ...
                <div className="text-center space-y-4">
                  ...
                  >
                    {isSignUp ? (
                      <span className="text-cmd-text-muted">Already a member? <span className="text-cmd-cyan hover:text-cmd-text-primary">Sign In</span></span>
                    ) : (
                      <span className="text-cmd-text-muted">New to Crinava? <span className="text-cmd-cyan hover:text-cmd-text-primary">Create Account</span></span>
                    )}
                  </button>
                </div>
              </div>
