...
export function PredictionGame({ onBack }: { onBack: () => void }) {
  const [view, setView] = useState<"splash" | "setup" | "onboard" | "game">(
    "splash",
  );
  ...
  const [activeTab, setActiveTab] = useState<
    "predict" | "results" | "oracle" | "stats" | "profile"
  >("predict");
  ...
  const genCode = () => {
    ...
    let s = "";
    for (let i = 0; i < 6; i += 1) s += c[Math.floor(Math.random() * c.length)];
    return s;
  };
  ...
  const syncSave = async (currentState: GameState) => {
    const data = {
      ...
      p1: currentState.p1,
      p2: currentState.p2,
      preds: currentState.preds,
      results: currentState.results,
      p1prof:
        currentState.me === "p1"
          ? {
              player: currentState.myProf.player,
              emoji: currentState.myProf.emoji,
            }
          : currentState.p1prof,
      p2prof:
        currentState.me === "p2"
  ...
  **/
  const syncLoad = async () => {
    if (!st.bid) return;
    try {
      const r = await fetch(`${BIN_URL}/${st.bid}/latest`, {
  ...
  /**
  * Creates a new game room, validates player names, saves the initial game state, and navigates to the onboarding view.
  * @example
  * sync()
  * undefined
  * @returns {Promise<void>} Resolves after the room is created and the game state is saved.
  **/
  ...
  /**
  * Joins a prediction game room by validating the join code, fetching the latest room state, and updating local game state.
  * @example
  * sync()
  * undefined
  * @param {void} - This function takes no arguments.
  * @returns {Promise<void>} Resolves after attempting to join the room and update state, or shows an error toast on failure.
  **/
  const handleJoin = async () => {
    ...
    try {
      const r = await fetch(`${BIN_URL}/${binId}/latest`, {
        headers: { "X-Master-Key": API_KEY },
      });
      const j = await r.json();
  ...
  if (view === "splash") {
    return (
      <motion.div className="fixed inset-0 z-50 bg-bg-primary flex flex-col items-center justify-center">
        <h1 className="text-6xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-500 to-accent-default tracking-[0.2em] uppercase italic">
          CRICARENA
        </h1>
        <p className="text-amber-500/60 tracking-[0.3em] mt-2 text-[10px] font-bold uppercase">
          IPL 2026 Edition
        </p>
        <button
          onClick={() => {
  ...
  if (loading) {
    return (
      <div className="w-full max-w-2xl mx-auto flex flex-col items-center justify-center py-32 space-y-6">
        <div className="text-amber-500 font-bold tracking-widest text-xl animate-pulse uppercase italic">
          {loading}
        </div>
  ...
  if (view === "setup") {
    return (
      <motion.div
        ...
        className="w-full max-w-md mx-auto space-y-8 pt-8"
      >
        <button
          onClick={onBack}
          className="flex items-center gap-2 text-fg-muted hover:text-fg-primary transition-colors mb-4"
        >
          <ArrowLeft size={18} />
        ...
        <div className="text-center mb-8">
          <h2 className="text-4xl font-bold text-amber-500 tracking-widest uppercase italic">
            CRICARENA
          </h2>
          <p className="text-fg-muted text-[10px] font-bold uppercase tracking-widest mt-2">
            PREDICT · COMPETE · WIN
          </p>
        </div>
        ...
        {!showJoin ? (
          <div className="premium-card p-8 space-y-6 shadow-2xl">
            <div className="space-y-2">
              <label className="text-[10px] text-fg-muted font-bold tracking-widest uppercase">
                Player 1 — You
              </label>
              <input
                value={p1Input}
                ...
                className="w-full bg-white/[0.03] border border-border-default rounded-xl px-4 py-3 text-sm font-bold text-fg-primary focus:border-amber-500 outline-none transition-colors"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] text-fg-muted font-bold tracking-widest uppercase">
            ...
            <div className="text-center text-fg-muted text-[10px] font-bold uppercase tracking-widest py-2">
              or
            </div>
            <button
              onClick={() => setShowJoin(true)}
              className="w-full border border-border-default text-fg-primary py-4 rounded-xl text-[10px] font-bold tracking-widest uppercase hover:border-amber-500 hover:text-amber-500 transition-colors"
            >
              JOIN EXISTING ROOM
            </button>
          </div>
          ...
          <div className="premium-card p-8 space-y-6 shadow-2xl">
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="Paste full code (e.g. ABCD12:abc123)"
                className="w-full bg-white/[0.03] border border-border-default rounded-xl px-4 py-3 text-sm font-bold text-fg-primary focus:border-amber-500 outline-none transition-colors font-mono"
              />
            </div>
            <button
              onClick={handleJoin}
            ...
            <button
              ...
              className="w-full border border-border-default text-fg-primary py-4 rounded-xl text-[10px] font-bold tracking-widest uppercase hover:border-white/40 transition-colors"
            >
              BACK
            </button>
          </div>
          ...
          {onboardStep === 0 && (
            <div className="space-y-6">
              <div className="text-center">
                <h3 className="text-2xl font-bold text-amber-500 tracking-widest uppercase italic">
                  YOUR LEGEND
                </h3>
                <p className="text-[10px] text-fg-muted font-bold mt-2 uppercase tracking-widest">
          ...
          {onboardStep === 1 && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-[10px] text-fg-muted font-bold mt-2 uppercase tracking-widest">
                  Pick an emoji for your profile.
                </p>
              </div>
          ...
          {onboardStep === 2 && (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-[10px] text-fg-muted font-bold mt-2 uppercase tracking-widest">
                  Create a 4-digit PIN to rejoin later.
                </p>
              </div>
              <input
                type="text"
              ...
              <div className="flex items-start gap-3 p-4 bg-amber-500/10 border border-amber-500/20 rounded-xl">
                ...
                <p className="text-[10px] text-amber-500 leading-relaxed font-medium">
                  Remember this PIN! You will need it to rejoin this room after
                  leaving.
                </p>
              </div>
              <button
  ...
  const otherEmoji =
    (st.me === "p1" ? st.p2prof?.emoji : st.p1prof?.emoji) || "😎";
    ...
    <div className="w-full max-w-2xl mx-auto space-y-6 pb-24">
      ...
      <div className="flex items-center justify-between premium-card p-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-sm font-bold text-amber-500 tracking-widest uppercase italic">
              CRICARENA
            </div>
            <div className="text-[9px] text-fg-muted font-bold uppercase tracking-widest">
              Room: {st.room}
            </div>
          </div>
        </div>
        ...
        <div className="premium-card p-6 relative overflow-hidden">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <div className="text-[9px] text-fg-muted tracking-widest uppercase font-bold">
                You
              </div>
            </div>
          </div>
          <div className="text-5xl font-bold text-amber-500 italic">
            {calcScore(st.me || "p1")}
          </div>
          <div className="text-[9px] text-fg-muted tracking-widest uppercase mt-2 font-bold">
            Correct Picks
          </div>
        </div>
        <div className="premium-card p-6 relative overflow-hidden">
          ...
          <div className="text-5xl font-bold text-fg-primary italic">
            {calcScore(st.me === "p1" ? "p2" : "p1")}
          </div>
          <div className="text-[9px] text-fg-muted tracking-widest uppercase mt-2 font-bold">
            Correct Picks
          </div>
      ...
      <div className="min-h-[400px]">
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center p-4 border-b border-border-default/50 bg-white/[0.01]">
                    <span className="text-xs font-bold text-amber-500 tracking-widest uppercase italic">
                      MATCH {m.id}
                    </span>
                    <div className="text-right">
                      <div className="text-[10px] text-fg-primary font-bold uppercase tracking-widest">
                        {m.date} · {m.day}
                      </div>
                      <div className="text-[9px] text-fg-muted font-bold uppercase tracking-widest">
                        {m.time}
                  ...
                  <div className="p-6">
                    <div className="grid grid-cols-[1fr_auto_1fr] gap-4 items-center">
                      <button
                        onClick={() =>
                          !myPick && !result && handleConfirmPick(m.id, m.h)
                        }
                      ...
                      <div className="text-[10px] font-bold text-fg-muted tracking-widest uppercase">
                        VS
                      </div>
                      <button
                        onClick={() =>
                          !myPick && !result && handleConfirmPick(m.id, m.a)
                        }
                  ...
                  <div className="flex gap-2 p-4 border-t border-border-default/50 bg-white/[0.01]">
                    <div
                      className={`flex-1 p-2 rounded-lg text-[10px] font-bold text-center border ${myPick ? "bg-amber-500/10 border-amber-500 text-amber-500" : "bg-white/[0.02] border-border-default text-fg-muted"}`}
                    >
                      <span className="block text-[8px] opacity-60 mb-1 uppercase tracking-widest">
                        {myName}
                      </span>
                      {myPick ? `${myPick} 🔒` : "—"}
                    </div>
                    <div
                      className={`flex-1 p-2 rounded-lg text-[10px] font-bold text-center border ${otherPick ? "bg-white/[0.03] border-border-default text-fg-primary" : "bg-white/[0.02] border-border-default text-fg-muted"}`}
                    >
                      <span className="block text-[8px] opacity-60 mb-1 uppercase tracking-widest">
                        {otherName}
                      </span>
        ...
        {activeTab === "results" && (
          <div className="space-y-4">
            {MATCHES.filter((m) => (st.results || {})[m.id]).length === 0 ? (
              <div className="text-center py-20 text-fg-muted text-[10px] font-bold uppercase tracking-widest">
                No results yet
              </div>
            ) : (
                ...
                return (
                  <div key={m.id} className="premium-card overflow-hidden">
                    <div className="flex justify-between items-center p-4 border-b border-border-default/50 bg-amber-500/5">
                      ...
                      <span className="text-[10px] font-bold text-amber-500 tracking-widest uppercase italic">
                        {w && TEAMS[w] ? TEAMS[w].short : "—"} WON 🏆
                      </span>
                    </div>
                    <div className="flex gap-2 p-4">
                      <div
                        className={`flex-1 p-3 rounded-xl text-[10px] font-bold text-center border ${p1ok ? "bg-amber-500/10 border-amber-500 text-amber-500" : "bg-red-500/10 border-red-500 text-red-500"}`}
                      >
                        <span className="block text-[8px] opacity-60 mb-1 uppercase tracking-widest">
                          {st.p1}
                        </span>
                      ...
                      <div
                        className={`flex-1 p-3 rounded-xl text-[10px] font-bold text-center border ${p2ok ? "bg-amber-500/10 border-amber-500 text-amber-500" : "bg-red-500/10 border-red-500 text-red-500"}`}
                      >
                        <span className="block text-[8px] opacity-60 mb-1 uppercase tracking-widest">
                          {st.p2}
                        </span>
        ...
        {(activeTab === "oracle" ||
          ...
          <div className="text-center py-20 premium-card">
            <p className="text-fg-muted text-[10px] font-bold uppercase tracking-widest italic">
