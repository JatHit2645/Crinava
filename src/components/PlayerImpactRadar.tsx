...
export const PlayerImpactRadar: React.FC<{ rawInfo: any, playerId: string, allPlayers?: string[] }> = ({ rawInfo, playerId: initialPlayerId, allPlayers = [] }) => {
  ...
  useEffect(() => {
    ...
    if (ballsFaced > 0) {
      ...
      setBattingData([
        { subject: 'Runs', A: Math.min(100, runsScored * 1.5), value: runsScored.toString(), fullMark: 100 },
        { subject: 'Strike Rate', A: Math.min(100, strikeRate / 2), value: strikeRate.toFixed(2), fullMark: 100 },
        { subject: 'Boundary %', A: Math.min(100, boundaryPct * 2.5), value: boundaryPct.toFixed(2) + '%', fullMark: 100 },
        { subject: 'Non-Dot %', A: Math.min(100, nonDotPct), value: nonDotPct.toFixed(2) + '%', fullMark: 100 },
        { subject: 'Balls Faced', A: Math.min(100, ballsFaced * 2), value: ballsFaced.toString(), fullMark: 100 },
      ]);
    } else {
      setBattingData([]);
    }
    ...
    if (ballsBowled > 0) {
      ...
      setBowlingData([
        { subject: 'Wickets', A: Math.min(100, wicketsTaken * 25), value: wicketsTaken.toString(), fullMark: 100 },
        { subject: 'Economy', A: economy > 0 ? Math.max(0, 100 - (economy * 8)) : 0, value: economy.toFixed(2), fullMark: 100 },
        { subject: 'Dot Ball %', A: Math.min(100, dotBallPct * 1.5), value: dotBallPct.toFixed(2) + '%', fullMark: 100 },
        { subject: 'Runs Conceded', A: Math.max(0, 100 - runsConceded * 1.5), value: runsConceded.toString(), fullMark: 100 },
        { subject: 'Balls Bowled', A: Math.min(100, ballsBowled * 2), value: ballsBowled.toString(), fullMark: 100 },
      ]);
    } else {
      setBowlingData([]);
    }
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-[#111] border border-[#333] p-3 rounded-lg shadow-xl">
          <p className="text-[#11EBCF] text-xs font-bold mb-1">{payload[0].payload.subject}</p>
          <p className="text-white text-sm font-black">{payload[0].payload.value}</p>
        </div>
      );
    }
    return null;
    ...
    <div className="bg-[#1a1a1a] p-6 rounded-xl border border-white/10 relative overflow-hidden group h-full">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h2 className="text-xl font-black text-white uppercase italic flex items-center gap-2">
            <Trophy size={20} className="text-aurora-teal" /> Player Impact Radar
          </h2>
      ...
      <div className="w-full">
        {loading ? (
          ...
          <div className={`grid grid-cols-1 ${battingData.length > 0 && bowlingData.length > 0 ? 'md:grid-cols-2' : ''} gap-4`}>
            {battingData.length > 0 && (
              <div className="h-[350px] flex flex-col items-center">
                <h3 className="text-aurora-teal font-black uppercase tracking-widest text-xs mb-2">Batting</h3>
                <ResponsiveContainer width="100%" height="100%">
                    <RadarChart cx="50%" cy="50%" outerRadius="55%" data={battingData}>
            ...
            {bowlingData.length > 0 && (
              <div className="h-[350px] flex flex-col items-center">
                <h3 className="text-metallic-gold font-black uppercase tracking-widest text-xs mb-2">Bowling</h3>
                <ResponsiveContainer width="100%" height="100%">
                  <RadarChart cx="50%" cy="50%" outerRadius="55%" data={bowlingData}>
                    <PolarGrid stroke="#333" />
                    <PolarAngleAxis 
                      dataKey="subject" 
                      tick={{fill: '#999', fontSize: 10, fontWeight: 'bold'}} 
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="p-4 bg-aurora-teal/5 rounded-xl border border-aurora-teal/20">
          <h4 className="text-aurora-teal font-black uppercase tracking-widest text-[10px] mb-2">What this shows</h4>
          <p className="text-xs text-gray-300 leading-relaxed">
            This radar chart breaks down <span className="text-white font-bold">{selectedPlayer}'s</span> contribution across <span className="text-aurora-teal font-bold uppercase">key dimensions</span>. A larger shape indicates a more dominant performance in that area.
          </p>
        </div>
        <div className="p-4 bg-metallic-gold/5 rounded-xl border border-metallic-gold/20">
          ...
          <p className="text-xs text-gray-300 leading-relaxed">
            A symmetrical shape shows a well-rounded <span className="text-metallic-gold font-bold uppercase">"Clutch"</span> performance that anchored the team's momentum during critical phases.
          </p>
        </div>
      </div>
    </div>
