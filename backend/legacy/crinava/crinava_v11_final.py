...
class CrinavaUnified:
    ...
    def load_and_verify(self, paths: Dict[str, str]) -> pd.DataFrame:
        log.info("[1/5] Loading & Verifying Matches (Final Column Sync)")
        frames = []
        for fmt, path in paths.items():
            ...
            for fpath in tqdm(files, desc=f"Processing {fmt}"):
                try:
                    ...
                    if not all(c in big_df.columns for c in REQUIRED_COLS): continue
                    ...
                    # Fix: Mapping your specific team columns to standard names
                    inn2["batting_team"] = inn2["team_2"]
                    inn2["bowling_team"] = inn2["team_1"]
    ...
    def build_twin_engines(self, full_df: pd.DataFrame):
        for fmt in ["T20", "ODI"]:
            ...
            n_tr = int(len(m_ids) * 0.85)
            tr_m, vl_m = m_ids[:n_tr], m_ids[n_tr:]
            tr_df_base, vl_df_base = f_df[f_df["match_id"].isin(tr_m)].copy(), f_df[f_df["match_id"].isin(vl_m)].copy()
            ...
            tr_df, vl_df = self._encode_categoricals(tr_df_base, fmt, fit=True), self._encode_categoricals(vl_df_base, fmt, fit=False)
            for col in CATEGORICAL_ENC:
                tr_df[col], vl_df[col] = pd.Categorical(tr_df[col]), pd.Categorical(vl_df[col])
            def prep_catboost(df_sub):
                ...
                return df_c
            log.info(f"Training Experts ({fmt})...")
            cb = CatBoostClassifier(iterations=600, depth=6, verbose=0, cat_features=CATEGORICAL_COLS).fit(prep_catboost(tr_df), tr_df["innings2_won"])
            self.env[fmt]["models"]["cb"] = cb
            dtr = xgb.DMatrix(tr_df[ALL_FEATURES], label=tr_df["innings2_won"], enable_categorical=True)
            self.env[fmt]["models"]["xgb"] = xgb.train({"objective": "binary:logistic", "seed": 42}, dtr, num_boost_round=300)
            lgb_tr = lgb.Dataset(tr_df[ALL_FEATURES], label=tr_df["innings2_won"])
            self.env[fmt]["models"]["lgb"] = lgb.train({"objective": "binary", "metric": "auc", "verbose": -1, "seed": 42}, lgb_tr, num_boost_round=300)
            numeric_tr = tr_df[ALL_FEATURES].copy()
            for col in CATEGORICAL_ENC: numeric_tr[col] = numeric_tr[col].astype(int)
            self.env[fmt]["models"]["rf"] = RandomForestClassifier(n_estimators=100, max_depth=10, n_jobs=-1, random_state=42).fit(numeric_tr.fillna(0), tr_df["innings2_won"])
            self.env[fmt]["models"]["lr"] = LogisticRegression(max_iter=1000).fit(numeric_tr.fillna(0), tr_df["innings2_won"])
            ...
            scaler.fit(tr_df[l_num].fillna(0))
            def build_seqs(df_sub):
                ...
                for _, mdf in df_sub.groupby("match_id", sort=False):
                    n_dat, c_dat = scaler.transform(mdf[l_num].fillna(0)), mdf[["phase_enc"]].fillna(0).values
                    data, lbl = np.hstack([n_dat, c_dat]).astype(np.float32), int(mdf["innings2_won"].iloc[0])
                    if len(data) >= SEQ_LENGTH:
                        for i in range(len(data)-SEQ_LENGTH+1): xs.append(data[i:i+SEQ_LENGTH]), ys.append(lbl)
            ...
            if len(tx) > 0:
                ...
                for _ in range(LSTM_EPOCHS):
                    ...
                    for xb, yb in ldr:
                        ...
                        opt.zero_grad(), crit(lstm(xb), yb).backward(), opt.step()
            self.env[fmt]["models"]["lstm"] = lstm.eval()
            p_mat = self._get_prob_matrix(vl_df, fmt)
            w = LinearRegression(fit_intercept=False, positive=True).fit(p_mat, vl_df["innings2_won"]).coef_.ravel()
    ...
    def _get_prob_matrix(self, df: pd.DataFrame, fmt: str) -> np.ndarray:
        ...
        df_cb[NUMERIC_FEATURES], df_cb[CATEGORICAL_COLS] = df_cb[NUMERIC_FEATURES].fillna(0), df_cb[CATEGORICAL_COLS].fillna("unknown").astype(str)
        ...
        p_xgb, p_lgb = m["xgb"].predict(xgb.DMatrix(df_tree, enable_categorical=True)), m["lgb"].predict(df_tree)
        df_num = df[ALL_FEATURES].copy()
        for col in CATEGORICAL_ENC: df_num[col] = df_num[col].astype(int)
        p_rf, p_lr = m["rf"].predict_proba(df_num.fillna(0))[:, 1], m["lr"].predict_proba(df_num.fillna(0))[:, 1]
        p_mc = np.zeros(len(df))
        for i, row in df.iterrows(): p_mc[i] = lookup_mc_prob(row["runs_needed"], row["balls_remaining"], row["wickets_left"], row["phase"], fmt, self.env[fmt]["mc_table"])
        p_ls = np.full(len(df), 0.5)
        scaler, l_num = self.env[fmt]["lstm_scaler"], [c for c in LSTM_FEATURE_COLS if c != "phase_enc"]
        for _, mdf in df.groupby("match_id", sort=False):
            idx, n_dat, c_dat = mdf.index, scaler.transform(mdf[l_num].fillna(0)), mdf[["phase_enc"]].fillna(0).values
            data, seqs = np.hstack([n_dat, c_dat]).astype(np.float32), []
            for i in range(len(data)):
                win = data[max(0, i-SEQ_LENGTH+1) : i+1]
                if len(win) < SEQ_LENGTH: win = np.vstack([np.zeros((SEQ_LENGTH-len(win), data.shape[1])), win])
                seqs.append(win)
            with torch.no_grad(): p_ls[idx] = m["lstm"](torch.tensor(np.array(seqs)).to(DEVICE)).cpu().numpy().ravel()
    ...
    def generate_outputs_and_save(self, full_df: pd.DataFrame):
        ...
        for fmt in ["T20", "ODI"]:
            f_df = full_df[full_df["fmt"] == fmt].copy().sort_values(["match_id", "over_no", "ball_no"]).reset_index(drop=True)
            if len(f_df) == 0: continue
            m_ids, tscv, f_df["oof_wp"] = f_df["match_id"].unique(), TimeSeriesSplit(n_splits=3), 0.5
            log.info(f"Generating OOF WPA for {fmt}...")
            enc = self._encode_categoricals(f_df, fmt, fit=False)
            for tr_idx, vl_idx in tqdm(tscv.split(m_ids), total=3):
                tr_m, vl_m = m_ids[tr_idx], m_ids[vl_idx]
                tr_oof, vl_oof = enc[enc["match_id"].isin(tr_m)].copy(), enc[enc["match_id"].isin(vl_m)].copy()
                tr_oof[NUMERIC_FEATURES], tr_oof[CATEGORICAL_COLS] = tr_oof[NUMERIC_FEATURES].fillna(0), tr_oof[CATEGORICAL_COLS].fillna("unknown").astype(str)
                vl_oof[NUMERIC_FEATURES], vl_oof[CATEGORICAL_COLS] = vl_oof[NUMERIC_FEATURES].fillna(0), vl_oof[CATEGORICAL_COLS].fillna("unknown").astype(str)
                tmp = CatBoostClassifier(iterations=200, depth=5, verbose=0, cat_features=CATEGORICAL_COLS).fit(tr_oof[CATBOOST_FEATURES], tr_oof["innings2_won"])
                f_df.loc[f_df["match_id"].isin(vl_m), "oof_wp"] = tmp.predict_proba(vl_oof[CATBOOST_FEATURES])[:, 1]
            f_df["wp_after"], f_df["wpa_delta"] = (f_df["oof_wp"] * 100).round(2), f_df.groupby("match_id")["wp_after"].diff().fillna(0).round(2)
            f_df[["match_id", "over_no", "ball_no", "wp_after", "wpa_delta"]].to_csv(self.out_dir/f"{fmt.lower()}_historical_wpa.csv", index=False)
            with open(self.out_dir/f"{fmt.lower()}_live_engine.json", "w") as fh: json.dump({"weights": self.env[fmt]["weights"].tolist(), "models": ["cb", "xgb", "lgb", "rf", "lr", "mc", "lstm"]}, fh)

if __name__ == "__main__":
    PATHS = {"T20": "/kaggle/input/datasets/jatin2645/white-ball-data/20_overs-20260429T154750Z-3-001/20_overs", "ODI": "/kaggle/input/datasets/jatin2645/white-ball-data/50_overs-20260429T154750Z-3-001/50_overs"}
    engine = CrinavaUnified()
    df = engine.load_and_verify(PATHS)
    engine.build_twin_engines(df)
    engine.generate_outputs_and_save(df)
    log.info("\n✅ CRINAVA V11.1 COMPLETE. SWEET DREAMS.")
