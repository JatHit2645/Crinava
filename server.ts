...
async function startServer() {
  ...
  app.post("/api/verdict", async (req, res) => {
    ...
    try {
      ...
      console.log("generateVerdict source:", generateVerdict.toString().substring(0, 300));
      console.log("====================================================");
  ...
  app.get("/api/test-nvidia-direct", async (req, res) => {
    ...
    const baseURL = (process.env.NVIDIA_API_URL || "https://integrate.api.nvidia.com/v1").trim().replace(/\/+$/, "");
  ...
  app.get("/api/matches/archive", async (req, res) => {
    ...
    try {
      const { getMatchArchive } = await import("./src/services/cockroachService");
      const matches = await getMatchArchive(Number(limit), Number(offset));
      res.json(matches);
    } catch (error: any) {
      console.error("Match archive error:", error);
      res.status(500).json({ error: "Failed to fetch match archive. Ensure 'pg' is installed." });
    }
  });
  ...
  app.get("/api/matches/:id/details", async (req, res) => {
    const { id } = req.params;
    try {
      const { getMatchDetails } = await import("./src/services/cockroachService");
  ...
  app.get("/api/series", async (req, res) => {
    try {
      const { getSeriesList } = await import("./src/services/cockroachService");
  ...
  app.get("/api/series/matches", async (req, res) => {
    ...
    try {
      const { getMatchesBySeries } = await import("./src/services/cockroachService");
      const matches = await getMatchesBySeries(String(eventName), String(season));
      res.json(matches);
    } catch (error: any) {
        ...
        const workers = Array(concurrencyLimit).fill(null).map(async () => {
          while (queue.length > 0 && !stopEnrichmentRequested) {
            ...
            while (!success && retryCount < maxRetries && !stopEnrichmentRequested) {
              try {
                const playerList = chunk.map((p: any, idx: number) => `${idx + 1}. ${p.player_name} (Gender: ${p.gender}, ID: ${p.player_id})`).join('\n');
  ...
  app.post("/api/ai-chat", async (req, res) => {
    console.log("POST /api/ai-chat called with body:", JSON.stringify(req.body).substring(0, 200));
