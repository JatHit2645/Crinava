...
print("=" * 60)
try:
    r = stealth_session.request("GET", f"https://api.goscorer.com/api/v3/getSC4?key={MATCH_KEY}", timeout=10)
    sc = r.json()
...
print("=" * 60)
try:
    r = stealth_session.request("GET", f"https://api.goscorer.com/api/v3/getSV3?key={MATCH_KEY}", timeout=10)
    sv = r.json()
...
map_urls = [
    ...
    f"https://oc.crickapi.com/mapping/getHomeMapData",
...
try:
    ...
    r = reqs.post(feeds_url, json=payload, headers=headers, impersonate="chrome120", timeout=5)
    if r.status_code == 200:
        feeds = r.json()
