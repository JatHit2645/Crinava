...
class CrinavaDiscovery:
    ...
    async def fetch_cricbuzz_schedule(self):
        """Step 1: Siphon matches across all three temporal states from Cricbuzz."""
        targets = {
        ...
        async def fetch_and_parse(state, url):
            try:
                self._print_once(f"siphon:{state}:{url}", f"[Discovery] Siphoning {state} from: {url}")
                resp = await asyncio.to_thread(stealth_session.request, "GET", url, timeout=15)
                ...
                matches = re.findall(pattern1, resp.text) + re.findall(pattern2, resp.text) + re.findall(pattern3, resp.text)
                for cb_id, slug in matches:
                    results[state].append((cb_id, slug))
            except Exception as e:
                self._print_once(f"discovery-error:{state}:{e}", f"[Error] Discovery failed for {state}: {e}")
        ...
        for state in ["Completed", "Upcoming", "Live"]:
            for cb_id, slug in results[state]:
                ...
                if "-vs-" in slug:
                    parts = slug.split("-vs-")
                    teams = [parts[0].split("-")[-1].lower(), parts[1].split("-")[0].lower()]
        ...
        count_signature = (
            len(final_matches),
            tuple((state, len(results[state])) for state in ["Live", "Completed", "Upcoming"]),
        )
    ...
    async def build_crex_map(self):
        ...
        for url in urls:
            try:
                ...
                self._print_once(f"crex-fetch:{url}", f"[Matching] Fetching match list from Crex URL: {url}")
                response = await asyncio.to_thread(stealth_session.request, "GET", url, timeout=20)
                if response.status_code != 200:
                    continue

                # Parse slugs and IDs using regex first to ensure we get everything
                pattern = r'href="/cricket-live-score/([^/]+-match-updates-([A-Za-z0-9]+))"'
                ...
                for slug, mid in matches:
                    if mid not in crex_map:
                        if "-vs-" in slug:
                            ...
                            crex_map[mid] = {
                                "t1_n": t1_n, "t1_sn": t1_n,
                                "t2_n": t2_n, "t2_sn": t2_n,
                                "title": pretty_title,
                                "slug": slug,
                ...
                for link in soup.find_all("a"):
                    ...
                    if mid in crex_map:
                        ...
                        if any(phrase in card_text for phrase in ["won by", "won the series", "result", "tied", "drawn", "abandoned", "no result", "complete", "won the match"]):
                        ...
                        # 2. Upcoming check (check next to prevent wrong Live tags)
                            ...
                            or bool(re.search(r'\b\d{1,2}:\d{2}\s*(?:am|pm)?\b', card_text))
                            or bool(re.search(r'\b\d{1,2}\s*(?:am|pm)\b', card_text))
                            or bool(re.search(r'\b(?:am|pm)\b', card_text))
                            or bool(re.search(r'\b(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\b', card_text))
                        ):
                            crex_map[mid]["state"] = "Upcoming"
                        # 3. Live check
                        elif "live" in card_text or "stumps" in card_text or "day " in card_text or "opt to" in card_text or "choose to" in card_text or "yet to bat" in card_text or bool(re.search(r'\(\d+\.\d+\)', card_text)):
                            crex_map[mid]["state"] = "Live"
                            
            except Exception as e:
                self._print_once(f"crex-error:{url}:{e}", f"[Error] Crex DOM siphon failed for {url}: {e}")
                
        return crex_map

    async def run_discovery_loop(self):
        """The main loop: Crex-Primary Discovery, then Cricbuzz-Hierarchy Fallback."""
        while True:
            ...
            ALIASES = {
                ...
                "phi": "phl", "phl": "phi",
                "yor": "yorks", "yorks": "yor",
                "nots": "notts", "notts": "nots",
                "warks": "warks", "warks": "warks",
                "ham": "hants", "hants": "ham",
                "lsg": "lucknow", "rr": "rajasthan",
                "rcb": "bengaluru", "dc": "delhi",
                "mi": "mumbai", "csk": "chennai",
                "kkr": "kolkata", "srh": "hyderabad",
                "pbks": "punjab", "gt": "gujarat",
                "pk": "punjab"
            }
            ...
            for crex_id, crex_data in crex_map.items():
                ...
                for cb in cb_matches:
                    ...
                    valid_crex_terms = {
                        crex_data["t1_n"], crex_data["t1_sn"],
                        crex_data["t2_n"], crex_data["t2_sn"]
                    }
                    ...
                    for cb_team in cb_teams:
                        ...
                        if clean_cb in clean_extended:
                            ...
                            for term in valid_crex_terms:
                                ...
                                if clean_term and (clean_cb in clean_term or clean_term.startswith(clean_cb) or clean_cb.startswith(clean_term)) and len(clean_cb) >= 3:
                                    team_matches += 1
                                    break
            ...
            if unmatched_count > 0:
                hierarchy_signature = tuple(sorted(m["match_id"] for m in final_list if m.get("source") == "hierarchy"))
                self._print_on_change(
                    "hierarchy-routes",
                    hierarchy_signature,
            ...
            cycle_signature = tuple(sorted((m.get("match_id"), m.get("state")) for m in final_list))
            self._print_on_change(
                "cycle-summary",
                cycle_signature,
