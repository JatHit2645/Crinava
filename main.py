...
from hub import load_cache, save_cache
...
async def verify_crinava_secret(request: Request, call_next):
    ...
    public_prefixes = ["/", "/favicon.ico", "/matches", "/scorecard/", "/history/", "/stream/"]
    if request.method == "OPTIONS" or any(request.url.path == p or request.url.path.startswith(p) for p in public_prefixes):
    ...
    is_local = "localhost" in origin or "127.0.0.1" in origin or "localhost" in referer or "127.0.0.1" in referer
    ...
    allowed_keys = {SECRET_KEY, "crinava_ultra_secure_123", "CRINAVA_2645_JATHIT_LIVE_SCORING_071108"}
    
    if not (is_local or is_same_host) and client_key not in allowed_keys:
        print(f"[Security] Access Denied for {request.url.path} from client key: {client_key}")
        return JSONResponse(status_code=403, content={"error": "Access Denied: Unrecognized Client. Anti-Scraping Active."})
...
async def list_matches():
            ...
            if mid in match_hub:
                if match_hub[mid].get("state") == "Completed":
                    ...
                    if history:
                        ...
                        if "won by" in score or "won by" in comm or "result" in score or "match tied" in score:
                            state = "Completed"
                        elif "yet to begin" in comm or "upcoming" in comm or "match not started" in score:
                            state = "Upcoming"
    ...
    match_list.sort(key=lambda x: 0 if x["state"] == "Live" else 1 if x["state"] == "Upcoming" else 2)
    return match_list
...
async def cache_status():
    ...
    for mid, data in match_hub.items():
        status[mid] = {
            ...
            "last_event": data["history"][-1].get("over_ball") if data.get("history") else None,
        }
...
async def orchestrator():
    ...
    while True:
        ...
        current_ids = {m.get("match_id") for m in discovery_engine.live_matches if "match_id" in m}
        ...
        for match in discovery_engine.live_matches:
            ...
            if match_id not in match_hub or not match_hub[match_id].get("worker"):
                ...
                match_hub[match_id]["state"] = "Completed" if previous_state == "Completed" else match.get("state", "Live")

                if match_hub[match_id].get("state") == "Completed":
                    continue

                print(f"[Orchestrator] Starting new worker for {match['title']} ({match['source']})...")
